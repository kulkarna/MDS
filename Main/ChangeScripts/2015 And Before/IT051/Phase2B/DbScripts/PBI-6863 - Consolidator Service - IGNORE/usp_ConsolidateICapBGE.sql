...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'usp_ConsolidateICapBGE')
    BEGIN
    	DROP PROCEDURE usp_ConsolidateICapBGE
    END
GO


CREATE PROCEDURE usp_ConsolidateICapBGE
AS
BEGIN
	SET NOCOUNT ON;
	...
	WHERE (AI.AccountID IS NULL OR AA.CapPLC > AI.StartDate ) 
		AND AA.CapPLC >= 0 
