...
-- IT121 - Update ScheduledSendDate of AccountSubmissionQueue table using the same logic of date_por_enrollment
--==================================================

ALTER procedure [dbo].[usp_account_renewal_upd]
(@p_username                                        nchar(100),
 ...
 @p_paymentTerm										int = 0		--Missing param --need to make parallel with usp_account_upd
 )  --added for IT002
as
...
UPDATE Libertypower..AccountSubmissionQueue
SET ScheduledSendDate =  @p_date_por_enrollment 
from Libertypower..AccountSubmissionQueue asq WITH (NOLOCK)
inner join Libertypower..AccountContractRate acr WITH (NOLOCK) on asq.AccountContractRateID = acr.AccountContractRateID
inner join Libertypower..AccountContract ac WITH (NOLOCK) on acr.AccountContractID = ac.AccountContractID
inner join Libertypower..Account a WITH (NOLOCK) on ac.AccountID = a.AccountID and ac.ContractID = a.CurrentRenewalContractID
where a.AccountIdLegacy = @p_account_id
and ac.ContractID = @w_CurrentContractID
and asq.AccountSubmissionQueueID = (select top 1 asq2.AccountSubmissionQueueID 
									from Libertypower..AccountSubmissionQueue asq2 WITH (NOLOCK)
									inner join Libertypower..AccountContractRate acr2 WITH (NOLOCK) on asq2.AccountContractRateID = acr2.AccountContractRateID
									where acr2.AccountContractID = ac.AccountContractID
									order by asq2.CreateDate desc)
-- END IT 121 - Diogo Lima
