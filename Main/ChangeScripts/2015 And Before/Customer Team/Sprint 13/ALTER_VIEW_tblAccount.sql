...
/****** Object:  View [dbo].[tblAccounts]    Script Date: 6/4/2015 3:31:40 PM ******/
SET ANSI_NULLS ON
GO
...
--===============================================
-- =============================================
-- Modified Bhavana Bakshi 
-- 1. Include new fields MostRecentAnnualUsage, MostRecentAnnualUsageDate
-- 2. Remove additional AnnualUsage LEFT Join
-- PBI 73127
--===============================================

ALTER VIEW [dbo].[tblAccounts]
AS
SELECT
		...
		USAGE.AnnualUsage AS AnnualUsage,--PBI 73127
		RECENTUSAGE.MostRecentAnnualUsage, RECENTUSAGE.MostRecentAnnualUsageDate,--PBI 73127
		NULL AS CreditScore,
	 ...
	 --   ASS.[Status]	AS status,
	  ...
	  --PBI 73127
	    LEFT JOIN(( 
			SELECT USAGE1.AccountID, USAGE1.AnnualUsage as MostRecentAnnualUsage, Usage1.EffectiveDate as MostRecentAnnualUsageDate
			FROM  LibertyPower.dbo.AccountUsage AS USAGE1 WITH (NOLOCK)	  
				  INNER JOIN (SELECT AccountID,MAx(Effectivedate) Effectivedate
						FROM  LibertyPower.dbo.AccountUsage   WITH (NOLOCK)	
						GROUP BY AccountID
						) USAGE2
				 ON USAGE1.AccountID = USAGE2.AccountID  AND USAGE1.EffectiveDate = USAGE2.Effectivedate)		
		)RECENTUSAGE  ON A.AccountID  = RECENTUSAGE.AccountID
...
-- NEW DEFAULT RATE JOIN:
		...
		--LEFT JOIN Libertypower.dbo.AccountUsage AU WITH (NOLOCK) ON A.AccountID = AU.AccountID AND C.StartDate = AU.EffectiveDate--PBI 73127 ( additional join not needed)
