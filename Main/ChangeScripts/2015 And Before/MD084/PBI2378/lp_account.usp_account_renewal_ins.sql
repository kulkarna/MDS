USE [lp_account]
GO
/****** Object:  StoredProcedure [dbo].[usp_account_renewal_ins]    Script Date: 11/15/2012 14:38:36 ******/
SET ANSI_NULLS ON
GO
...
-- When we have add on accounts in renewal, the inserted account would take care of the AccountContract record so we need to check if that hapenned
		...
		WHILE (SELECT COUNT(MultiTermID) FROM @MultiTermTable) > 0
			BEGIN
				...
				SELECT	@ACR_RateStart		= StartDate,
						...
						@ACR_Rate			= CASE WHEN @MtCounter = 0 THEN @ACR_Rate ELSE Price END, -- first term can be adjusted by ABC channels
						@ACR_Term			= Term,
