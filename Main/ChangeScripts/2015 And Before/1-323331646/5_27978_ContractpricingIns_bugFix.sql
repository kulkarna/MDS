USE [lp_deal_capture]
GO
/****** Object:  StoredProcedure [dbo].[usp_contract_pricing_ins]    Script Date: 12/17/2013 09:29:39 ******/
SET ANSI_NULLS ON
GO
...
-- SR 1-323331646
-- Description:	Removed AccountNumber parameter from ufn_GetMinStartDateforanAccount call
-- =============================================
-- Modified by:	Isabelle Tamanini
-- Modified date: 01/08/2014
-- SR 1-323331646
-- Description:	Logic to calculate and update renewal start date for each account
-- was moved to the contract submission
-- =============================================
-- Modify : Sara lakshmanan
-- Date : jan 24 2014
-- Ticket: Bug fix: Added the update logic to insert the correct contract start date for all the accounts. 
--Since the CRMConversion is setting the ContractStartDate based on the Accounts effective startDate
-- =============================================
...
--Jan 08  2014
--Logic was moved to the contract submission
...
--Set @w_account_date_start=(Select lp_deal_capture.dbo.ufn_GetMinStartDateforanAccount(@AccountID, @p_contract_eff_start_date, @p_utility_id))
...
--Set @w_account_date_start1=(Select lp_deal_capture.dbo.ufn_GetMinStartDateforanAccount(@AccountID,  @p_contract_eff_start_date, @p_utility_id))
...
-----------------------------------------------------------------------------------------------------
--Adding the Logic back as it is affecting the Contract Start dates
--That is used in the CRM Conversion : The contract Start is based on the accounts effective start date
--Added Jan 24 2013 - 28978: OnCor Root Cause fix
