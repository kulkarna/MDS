...
IF OBJECT_ID('Workspace..UsageConsolidated_BANGOR') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_BANGOR

SELECT * INTO Workspace..UsageConsolidated_BANGOR FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'BANGOR'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('BANGOR')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('BANGOR') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'BANGOR' 
ORDER BY RowNumber
