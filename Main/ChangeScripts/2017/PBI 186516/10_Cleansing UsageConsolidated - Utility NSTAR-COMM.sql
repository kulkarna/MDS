...
IF OBJECT_ID('Workspace..UsageConsolidated_NSTARCOMM') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_NSTARCOMM

SELECT * INTO Workspace..UsageConsolidated_NSTARCOMM FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'NSTAR-COMM'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('NSTAR-COMM')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('NSTAR-COMM') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'NSTAR-COMM' 
ORDER BY RowNumber
