...
IF OBJECT_ID('Workspace..UsageConsolidated_UNITIL') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_UNITIL

SELECT * INTO Workspace..UsageConsolidated_UNITIL FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'UNITIL'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('UNITIL')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('UNITIL') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'UNITIL' 
ORDER BY RowNumber
