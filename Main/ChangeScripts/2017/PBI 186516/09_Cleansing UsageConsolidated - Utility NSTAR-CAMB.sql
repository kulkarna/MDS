...
IF OBJECT_ID('Workspace..UsageConsolidated_NSTARCAMB') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_NSTARCAMB

SELECT * INTO Workspace..UsageConsolidated_NSTARCAMB FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'NSTAR-CAMB'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('NSTAR-CAMB')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('NSTAR-CAMB') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'NSTAR-CAMB' 
ORDER BY RowNumber
