...
IF OBJECT_ID('Workspace..UsageConsolidated_WMECO') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_WMECO

SELECT * INTO Workspace..UsageConsolidated_WMECO FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'WMECO'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('WMECO')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('WMECO') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'WMECO' 
ORDER BY RowNumber
