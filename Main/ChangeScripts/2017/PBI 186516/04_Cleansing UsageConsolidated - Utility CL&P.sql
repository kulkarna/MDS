...
IF OBJECT_ID('Workspace..UsageConsolidated_CLP') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_CLP

SELECT * INTO Workspace..UsageConsolidated_CLP FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'CL&P'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('CL&P')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('CL&P') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'CL&P' 
ORDER BY RowNumber
