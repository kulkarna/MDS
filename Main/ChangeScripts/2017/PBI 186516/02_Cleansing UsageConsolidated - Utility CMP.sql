...
IF OBJECT_ID('Workspace..UsageConsolidated_CMP') IS NOT NULL
		DROP TABLE  Workspace..UsageConsolidated_CMP

SELECT * INTO Workspace..UsageConsolidated_CMP FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'CMP'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('CMP')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('CMP') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'CMP' 
ORDER BY RowNumber
