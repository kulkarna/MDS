...
IF OBJECT_ID('Workspace..UsageConsolidated_MECO') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_MECO

SELECT * INTO Workspace..UsageConsolidated_MECO FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'MECO'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('MECO')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('MECO') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'MECO' 
ORDER BY RowNumber
