...
IF OBJECT_ID('Workspace..UsageConsolidated_UI') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_UI

SELECT * INTO Workspace..UsageConsolidated_UI FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'UI'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('UI')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('UI') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'UI' 
ORDER BY RowNumber
