...
IF OBJECT_ID('Workspace..UsageConsolidated_NSTARBOS') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_NSTARBOS

SELECT * INTO Workspace..UsageConsolidated_NSTARBOS FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'NSTAR-BOS'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('NSTAR-BOS')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('NSTAR-BOS') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'NSTAR-BOS' 
ORDER BY RowNumber
