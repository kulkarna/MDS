...
IF OBJECT_ID('Workspace..UsageConsolidated_NECO') IS NOT NULL
		DROP TABLE Workspace..UsageConsolidated_NECO

SELECT * INTO Workspace..UsageConsolidated_NECO FROM Libertypower..UsageConsolidated uc
WHERE uc.UtilityCode = 'NECO'
...
FROM Libertypower..UsageConsolidated(nolock) UC WHERE UC.UtilityCode IN ('NECO')
ORDER BY RowNumber
...
WHERE UC.UtilityCode IN('NECO') AND AccountNumber = @ActualAccount
...
ON uc.ID = cm.ID WHERE uc.UtilityCode = 'NECO' 
ORDER BY RowNumber
