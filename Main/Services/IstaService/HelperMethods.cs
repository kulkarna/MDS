...
namespace LibertyPower.DataAccess.WebServiceAccess.IstaWebService
{
	...
	public static class HelperMethods
	{
		...
		public static IstaEnrollmentService.EnrollCustomer BuildCustomer( DataRow accountData, DateTime startDate, string enrollmentType )
		{
            ...
            //PBI-89841 -Populating Dummy value for rate code as  ISTA errors out if null
            string rateCode = accountData["ratecode"].ToString();
            if (string.IsNullOrWhiteSpace(rateCode))
                product.LDCRateCode = "Dummy";
            else
                product.LDCRateCode = rateCode;
            ...
            //PBI-89841 -Populating Dummy value for rate code as  ISTA errors out if null
            string defalutRateCode = accountData["defaultratecode"].ToString();
            if (string.IsNullOrWhiteSpace(defalutRateCode))
                defaultProduct.LDCRateCode = "Dummy";
            else
                defaultProduct.LDCRateCode = defalutRateCode;
        ...
        /// <summary>
        /// Returns the string value of the column of the supplied data-row.
        /// If the supplied data-row is null or the column is null, returns null.
        /// </summary>
        /// <param name="dataRow">The source data-row.</param>
        /// <param name="columnName">The source column.</param>
        /// <returns>The string value or null.</returns>
        public static string GetStringValueFromColumnOfDataRow(DataRow dataRow, string columnName)
        {
            if (dataRow == null)
                return null;

            return dataRow.IsNull(columnName) ? null : dataRow[columnName].ToString();
        }

        /// <summary>
        /// Returns the integer value stored in the column of the data-row.
        /// if the value is null, empty, white-spaces or invalid, returns zero.
        /// </summary>
        /// <param name="dataRow">The source data-row.</param>
        /// <param name="columnName">The source column.</param>
        /// <returns>The integer value or zero.</returns>
        public static int GetIntValueFromColumnOfDataRow(DataRow dataRow, string columnName)
        {
            var stringValue = GetStringValueFromColumnOfDataRow(dataRow, columnName);
        ...
        /// <summary>
        /// Validates the reponse from a Request Service method.
        /// If any errors are found, throws an exception with all error messages concatened.
        /// </summary>
        /// <param name="response">The response object.</param>
        public static void ValidateRequestServiceResponse(RequestService.ResponseDTO[] response)
        {
            if (response == null || response.Length == 0)
                throw new Exception("The Request Service did not return a response for the request.");

            var responseContainsError = response.Any(x => x.MessageCode != "0000");
            if (responseContainsError)
            {
                var template = "\t- {0}: {1};" + Environment.NewLine;

                var i = 1;
                var messages =
                    response
                    .Select(x => string.Format(template, i++, x.Message))
                    .Aggregate((x, y) => x + y);

                throw new Exception("The request could not be sent. Reason(s):" + Environment.NewLine + messages);
            }
        }
	}
}
