...
namespace UtilityManagement.Controllers
{
    ...
    public class AuditMeterTypeController : ControllerBaseWithoutUtilDropDown
    {
        #region private variables
        private const string CLASS = "AuditMeterTypeController";
        #endregion


        #region public constructors
        public AuditMeterTypeController() : base()
        {
            ViewBag.PageName = "AuditMeterType";
            ViewBag.IndexPageName = "AuditMeterType";
            ViewBag.PageDisplayName = "Audit Meter Type";
        }
        #endregion
        ...
        public ActionResult Index()
        {
            ...
            catch (Exception exc)
            {
                ...
                return View(new List<usp_zAuditMeterType_SELECT_Result>());
            }
        }
        ...
        //
        // GET: /AuditMeterTypeController/Details/5
        public ActionResult Details(Guid id)
        {
            usp_zAuditMeterType_SELECT_Result usp_zAuditMeterType_SELECT_Result = _db.usp_zAuditMeterType_SELECT().Where(x => x.Id == id).FirstOrDefault();
            if (usp_zAuditMeterType_SELECT_Result == null)
            {
                return HttpNotFound();
            }
            return View(usp_zAuditMeterType_SELECT_Result);
        }
        ...
        public ActionResult MeterTypeCodeClick()
        {
            string method = "MeterTypeCodeClick()";
            try
            {
                VerifyMessageIdAndErrorMessageSession();
                _logger.LogInfo(Session[Common.MESSAGEID].ToString(), string.Format("{0}.{1}.{2} {3}", Common.NAMESPACE, CLASS, method, Common.BEGIN));
        ...
        public ActionResult MeterTypeCodePrevClick()
        {
            string method = "MeterTypeCodePrevClick()";
            try
            {
                VerifyMessageIdAndErrorMessageSession();
                _logger.LogInfo(Session[Common.MESSAGEID].ToString(), string.Format("{0}.{1}.{2} {3}", Common.NAMESPACE, CLASS, method, Common.BEGIN));

                var response = ManageSortationSession("MeterTypeCodePrev");
        ...
        public ActionResult LpStandardMeterTypeClick()
        {
            string method = "LpStandardMeterTypeClick()";
            try
            {
                VerifyMessageIdAndErrorMessageSession();
                _logger.LogInfo(Session[Common.MESSAGEID].ToString(), string.Format("{0}.{1}.{2} {3}", Common.NAMESPACE, CLASS, method, Common.BEGIN));
        ...
        public ActionResult LpStandardMeterTypePrevClick()
        {
            string method = "LpStandardMeterTypePrevClick()";
            try
            {
                VerifyMessageIdAndErrorMessageSession();
                _logger.LogInfo(Session[Common.MESSAGEID].ToString(), string.Format("{0}.{1}.{2} {3}", Common.NAMESPACE, CLASS, method, Common.BEGIN));

                var response = ManageSortationSession("LpStandardMeterTypePrev");
        ...
        private List<usp_zAuditMeterType_SELECT_Result> ObtainResponse()
        {
            var response = _db.usp_zAuditMeterType_SELECT().OrderBy(x => x.Id).ToList();
            ...
            switch (Session[Common.SORTCOLUMNNAME].ToString())
            {
                ...
                case "MeterTypeCode":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.MeterTypeCode).ToList();
                    else
                        response = response.OrderBy(x => x.MeterTypeCode).ToList();
                    break;
                case "MeterTypeCodePrevious":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.MeterTypeCodePrevious).ToList();
                    else
                        response = response.OrderBy(x => x.MeterTypeCodePrevious).ToList();
                    break;
                case "Description":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                ...
                case "LpStandardMeterType":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.LpStandardMeterType).ToList();
                    else
                        response = response.OrderBy(x => x.LpStandardMeterType).ToList();
                    break;
                case "LpStandardMeterTypePrevious":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.LpStandardMeterTypePrevious).ToList();
                    else
                        response = response.OrderBy(x => x.LpStandardMeterTypePrevious).ToList();
                    break;
                case "Inactive":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
