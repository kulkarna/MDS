...
{
    public class RateClassTabsBusinessLayer : TabsBusinessLayer
    {
        #region private variables
        private string CLASS = "RateClassTabsBusinessLayer";
        #endregion

        #region public constructors
        public RateClassTabsBusinessLayer(string messageId, IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger, DataSet dataSet, string userName)
            : base(messageId, dataRepository, excelWorksheetUtility, logger, dataSet, userName)
        {
            string method = string.Format("RateClassTabsBusinessLayer(messageId,dataRepository,excelWorksheetUtility,logger,dataSet,userName:{0})",Common.NullSafeString(userName));
            try
            {
                ...
                if (_dataSet != null && _dataSet.Tables != null && _dataSet.Tables.Count > 0 && _dataSet.Tables[0] != null && _dataSet.Tables[0].Rows != null)
                {
                    foreach (DataTable dataTable in _dataSet.Tables)
                    {
                        if (dataTable != null && !string.IsNullOrWhiteSpace(dataTable.TableName))
                        {
                            ...
                            {
                                case "Rate Class":
                                    ITabBusinessLayer rateClassTabBusinessLayer = new RateClassTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(rateClassTabBusinessLayer);
                                    break;
                                case "LP Std Rate Class":
                                    ITabBusinessLayer lpStandardRateClassTabBusinessLayer = new LpStandardRateClassTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(lpStandardRateClassTabBusinessLayer);
                                    break;
                                case "Rate Class Alias":
                                    ITabBusinessLayer rateClassAliasTabBusinessLayer = new RateClassAliasTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(rateClassAliasTabBusinessLayer);
                                    break;
                            }
                        }
