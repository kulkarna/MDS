...
{
    public class LpStandardTariffTabCodeBusinessLayer : TabBusinessLayer
    {
        #region private variables
        private string CLASS = "LpStandardTariffTabCodeBusinessLayer";
        const string LP_STANDARD_TARIFF_CODE_CODE = "LpStandardTariffCodeCode";
        protected const string TAB_NAME = "LP Std Tariff Code";
        #endregion

        #region public constructors
        public LpStandardTariffTabCodeBusinessLayer(string messageId, IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger, DataTable dataTable, List<DataAccessLayerEntityFramework.UtilityCompany> utilityCompanies)
            : base(messageId, dataRepository, excelWorksheetUtility, logger, TAB_NAME, utilityCompanies)
        {
            string method = string.Format("{0}.{1}.LpStandardTariffTabCodeBusinessLayer(messageId,dataRepository,excelWorksheetUtility,logger,dataTable)", NAMESPACE, CLASS);
            try
            {
                _logger.LogInfo(messageId, string.Format("{0}.{1}.{2} {3}", NAMESPACE, CLASS, method, Common.BEGIN));

                Columns.Add(UTILITY_CODE);
                Columns.Add(LP_STANDARD_TARIFF_CODE_CODE);
                    ...
                    foreach (DataRow dataRow in _dataTable.Rows)
                    {
                        if (rowCount < _dataTable.Rows.Count)
                        {
                            ...
                            )
                            {
                                ...
                                try
                                {
                                    DataSet dataSet = _dataRepository.usp_LpStandardTariffCode_UPSERT(messageId, Guid.NewGuid().ToString(),
                                        Common.NullSafeString(dataRow[UTILITY_CODE]),
                                        Common.NullSafeString(dataRow[LP_STANDARD_TARIFF_CODE_CODE]),
                                        inactive,
                                        userName);
        ...
        protected override bool DoesDataTableHaveAllValidColumns(string messageId)
        {
            ...
            try
            {
                ...
                returnValue = _dataTable != null
                    ...
                    && DoesDataTableHaveColumn(messageId, LP_STANDARD_TARIFF_CODE_CODE)
