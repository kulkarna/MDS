...
{
    public class MeterReadCalendarTabsBusinessLayer : TabsBusinessLayer
    {
        #region private variables
        private string CLASS = "MeterReadCalendarTabsBusinessLayer";
        #endregion

        #region public constructors
        public MeterReadCalendarTabsBusinessLayer(string messageId, IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger, DataSet dataSet, string userName)
            : base(messageId, dataRepository, excelWorksheetUtility, logger, dataSet, userName)
        {
            string method = "MeterReadCalendarTabsBusinessLayer(messageId,dataRepository,excelWorksheetUtility,logger,dataSet,userName)";
            try
            {
                ...
                if (_dataSet != null && _dataSet.Tables != null && _dataSet.Tables.Count > 0 && _dataSet.Tables[0] != null && _dataSet.Tables[0].Rows != null)
                {
                    foreach (DataTable dataTable in _dataSet.Tables)
                    {
                        if (dataTable != null && !string.IsNullOrWhiteSpace(dataTable.TableName))
                        {
                            ...
                            {
                                case "Meter Read Calendar":
                                    ITabBusinessLayer meterReadCalendarTabBusinessLayer = new MeterReadCalendarTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(meterReadCalendarTabBusinessLayer);
                                    break;
                            }
                        }
                    }
