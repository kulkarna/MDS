...
-- =============================================
CREATE PROCEDURE [dbo].[usp_VRE_MisoDayAheadSelect]
...
BEGIN
	...
	BEGIN
		...
		AND ([Node]  IN (SELECT zoneId FROM @tempZone) OR @useList = 0 )
	END
	ELSE
	BEGIN
		SELECT [ID]
		  ...
		  ,[DateModified]
		  ,[ModifiedBy]
		FROM [VREMisoDayAhead] 
		WHERE ID IN(
				SELECT MAX(DA.ID) 
				FROM [VREMisoDayAhead] AS DA 
				WHERE 
				FileContextGUID = ISNULL(@FileContextGuid,FileContextGuid)
				AND	ISNULL(@StartDate, DA.[Date]) <= DA.[Date]
				AND   ISNULL(@EndDate, DATEADD(DAY,1,DA.[Date]) ) > DA.[Date]
				AND ([Node]  IN (SELECT zoneId FROM @tempZone) OR @useList = 0 )
				GROUP BY DA.Node, DA.[Date]
			);
	END
...
GO
EXECUTE sp_addextendedproperty @name = N'VirtualFolder_Path', @value = N'VRE', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_VRE_MisoDayAheadSelect';
