 ...
 */
CREATE PROCEDURE [dbo].[usp_AccountByAccountIdSelect_eric]                                                                                     
	@AccountId	char(12)
AS
BEGIN
    SET NOCOUNT ON;
	...
	SELECT	DISTINCT 
			...
			, retail_mkt_id AS RetailMarketCode, a.zone As ZoneCode, service_rate_class AS RateClass
	...
	*/
	...
	-- This next section was added so that the stored proc can accept both the legacy and newere Account IDs.
    -- Note that the legacy account_id is not numeric.
	DECLARE @INT_AccountID INT
	DECLARE @CHAR_AccountID CHAR(12)
    IF ISNUMERIC(@AccountId) = 0
    BEGIN
		SET @INT_AccountID = NULL
		SET @CHAR_AccountID = @AccountId
	END
	ELSE
	BEGIN
		SET @INT_AccountID = CAST(@AccountId AS INT);
		SET @CHAR_AccountID = NULL
	END
	
	--INSERT INTO #Account
	--SELECT AccountID INT, AccountIdLegacy VARCHAR(12), AccountNumber VARCHAR(30), PorOption BIT, icap VARCHAR(15), tcap VARCHAR(15), BillingGroup VARCHAR(15), Zone VARCHAR(50), ServiceRateClass VARCHAR(50), AccountTypeID INT, CustomerID INT, CurrentContractID INT, UtilityID INT, RetailMktID INT
	SELECT AccountID, AccountIdLegacy, AccountNumber, PorOption, icap, tcap, BillingGroup, Zone, ServiceRateClass, AccountTypeID, CustomerID, CurrentContractID, UtilityID, RetailMktID, BillingTypeID
	INTO #Account
	FROM LibertyPower.dbo.Account (NOLOCK)
	WHERE (@CHAR_AccountID IS NULL OR AccountIdLegacy = @CHAR_AccountID)
	AND (@INT_AccountID IS NULL OR AccountID = @INT_AccountID)
	...
	SELECT	DISTINCT 
			...
			, LibertyPower.dbo.[ufn_GetLegacyDateDeenrollment](ACS.[Status], ACS.[SubStatus], ASERVICE.StartDate) AS DeenrollmentDate
			...
			, CUST_NAME.full_name As BusinessName
	...
	FROM 
		...
		JOIN LibertyPower..AccountContract AC WITH (NOLOCK) ON A.CurrentContractID = AC.ContractID AND A.AccountId = AC.AccountID -- current/lasdt contract
		...
		JOIN LibertyPower..AccountUsage USAGE WITH (NOLOCK) ON A.AccountID = USAGE.ACcountID AND USAGE.EffectiveDate = C.StartDate
		JOIN LibertyPower.dbo.UsageReqStatus URS	WITH (NOLOCK)		ON USAGE.UsageReqStatusID = URS.UsageReqStatusID
		LEFT JOIN lp_account..account_name CUST_NAME WITH (NOLOCK) ON CUST_NAME.AccountNameID = CUST.NameID
		...
		LEFT JOIN LibertyPower..AccountEtfWaive AW 	WITH (NOLOCK) ON A.AccountId = AW.AccountID 
		
		LEFT JOIN lp_account..account_additional_info i WITH (NOLOCK) ON A.AccountIdLegacy = i.account_id
