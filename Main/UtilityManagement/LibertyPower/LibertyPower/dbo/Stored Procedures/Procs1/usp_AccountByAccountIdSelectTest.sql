/*******************************************************************************
* usp_AccountByAccountIdSelectTest
...
CREATE PROCEDURE [dbo].[usp_AccountByAccountIdSelectTest]                                                                                     
	@AccountId	char(12)
AS
BEGIN
    SET NOCOUNT ON;
	...
	*/
	...
	SELECT DISTINCT
			...
			, ISNULL(USAGE.AnnualUsage, (SELECT TOP 1 ISNULL(AnnualUsage, 0) FROM AccountUsage WHERE AccountID = A.AccountID ORDER BY EffectiveDate DESC)) AS AnnualUsage
