...
exec [usp_DailyPricingTermsSelect_prod] @p_username = 'libertypower\jmunoz', @p_product_id= 'ONCOR_FX', @p_account_type= 'SMB', @p_contract_date='20120430'
exec [usp_DailyPricingTermsSelect_prod_jmunoz] @p_username = 'libertypower\jmunoz', @p_product_id= 'ONCOR_FX', @p_account_type= 'SMB', @p_contract_date='20120430'


 */
CREATE PROCEDURE [dbo].[usp_DailyPricingTermsSelect_prod_jmunoz]
...
BEGIN
	...
	SELECT	@SalesChannel	= REPLACE(@p_username , 'libertypower\', '') 
			,@StartMonth	= DATEPART(mm, @p_contract_eff_start_date)
			,@StartYear		= DATEPART(yyyy, @p_contract_eff_start_date)
	...
	FROM	#common_product_rate pr WITH (NOLOCK)
			...
			INNER JOIN libertypower..utility u WITH (NOLOCK) ON p.utility_id  = u.UtilityCode		
			...
			AND (
				 ...
				 OR dp.sales_channel_role	in 
					(	SELECT b.RoleName 
						...
						JOIN libertypower..[Role] b WITH (NOLOCK) 
						ON b.RoleID			= a.RoleID  
						JOIN libertypower..[User] u with (NOLOCK INDEX = User__Username_I)  
