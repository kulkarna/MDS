...
CREATE procedure [dbo].[usp_rate_sel_listbyusername_bak]
...
BEGIN 
	...
	DECLARE @w_sales_channel_role varchar(50) 
	...
	begin 
		...
		SELECT distinct rate_id from deal_contract_print where contract_nbr = @p_contract_nbr and product_id = @p_product_id
...
--		SELECT distinct rate_id from deal_contract_account where contract_nbr = @p_contract_nbr and product_id = @p_product_id
		...
		INSERT INTO @selected_rate 
		SELECT distinct rate_id from multi_rates where contract_nbr = @p_contract_nbr and product_id = @p_product_id
	...
	FROM lp_common..common_product_rate pr with (NOLOCK INDEX = common_product_rate_idx)
		...
		LEFT JOIN lp_deal_capture..deal_pricing_detail dpd ON dpd.product_id = pr.product_id 
    ...
    WHERE pr.product_id = @p_product_id
		...
		(
			...
			(
				...
				JOIN lp_common..common_product p2 ON pr2.product_id = p2.product_id 
				LEFT JOIN lp_deal_capture..deal_pricing_detail dpd2 ON dpd2.product_id = pr2.product_id 
	...
	BEGIN		
		...
		WHERE ph.contract_eff_start_date		>= dateadd(mm,datediff(mm,0,@p_contract_eff_start_date),0)
			...
			(
				...
				from lp_common.dbo.product_rate_history 
    ...
    WHERE pr.product_id = @p_product_id
		...
		AND ( dpd.rate_submit_ind = 0 
				OR  EXISTS ( Select contract_nbr from deal_contract where contract_nbr = @p_contract_nbr and @p_contract_nbr <> '' and product_id = dpd.product_id and rate_id = dpd.rate_id 
								UNION 
