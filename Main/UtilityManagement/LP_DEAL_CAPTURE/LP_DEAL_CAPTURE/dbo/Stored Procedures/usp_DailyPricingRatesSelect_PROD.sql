...
CREATE PROCEDURE [dbo].[usp_DailyPricingRatesSelect_PROD]
	@Username					nchar(100),
...
---------- 	Product Rate IDs  -----------------------------------------------------------------------------------------	
    ...
    ELSE
            ...
            JOIN lp_common..product_rate_history ph WITH (NOLOCK) 
...
---------- 	Product Rates  --------------------------------------------------------------------------------------------				
	...
	FROM	#common_product_rate r WITH (NOLOCK)
			...
			INNER JOIN lp_common..common_utility u WITH (NOLOCK) 
			ON p.utility_id  = u.utility_id		
	...
	WHERE	--r.term_months		= @p_term_months
			r.term_months		= isnull(@p_term_months,r.term_months) -- Ticket 17672 
	AND		r.product_id		= @p_product_id
	AND		r.Inactive_ind		= 0
--  AND		pc.ChannelTypeID	= isnull(@ChannelTypeID, pc.ChannelTypeID) -- Ticket 17672 
	AND		(
				pc.ChannelTypeID	= isnull(@ChannelTypeID, pc.ChannelTypeID) -- Ticket 17672 	 -- Type is equal to  Channel's type.
	...
	AND		LEFT(r.rate_id, 3) % 100	= case when @ChannelGroupID is null then LEFT(r.rate_id, 3) % 100
								else @ChannelGroupID end -- Ticket 17672 
	AND 
	(
		@p_show_startdate_flag = 1 OR -- if this flag is turned on, then show all future rates
		DATEPART(mm,r.contract_eff_start_date) = 
		(
			SELECT TOP 1 DATEPART(mm,pr2.contract_eff_start_date )
			FROM #common_product_rate pr2 WITH (NOLOCK)
			JOIN lp_common..common_product p2 WITH (NOLOCK) ON pr2.product_id = p2.product_id 
			...
			WHERE pr2.product_id = @p_product_id
				AND pr2.term_months = isnull(@p_term_months,r.term_months)
				AND pr2.eff_date <= convert(char(08), getdate(), 112) 
--				AND pr2.due_date >= convert(char(08), getdate(), 112) 
				AND pr2.inactive_ind = isnull(@p_show_active_flag,r.inactive_ind)
				...
				END 
			ORDER BY pr2.contract_eff_start_date
		)
	)	
    ...
    WHERE	pr.term_months	= isnull(@p_term_months, pr.term_months) -- Ticket 17672 
	...
	AND		(	-- user has access to custom rate	
				...
				(	SELECT	b.RoleName 
					FROM	lp_portal..UserRoles a WITH (NOLOCK)
							JOIN lp_portal..Roles b  WITH (NOLOCK)
