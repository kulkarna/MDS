...
*/
CREATE procedure [dbo].[usp_deal_pricing_product_sel_list_by_user_jmunoz]
(
...
BEGIN 
   ...
   from lp_common..common_product			p with (NOLOCK INDEX = common_product_idx) 
	JOIN lp_security..security_role_product rp WITH (NOLOCK)	ON rp.product_id	= p.product_id
	JOIN libertypower..[UserRole]			ur WITH (NOLOCK)	ON ur.roleid		= rp.role_id
	JOIN libertypower..[User]				u WITH (NOLOCK)		ON u.userid			= ur.userid
   where
