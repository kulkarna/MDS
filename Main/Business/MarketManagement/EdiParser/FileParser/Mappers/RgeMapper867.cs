namespace LibertyPower.Business.MarketManagement.EdiParser.FileParser
{
	...
	/// </summary>
	public class RgeMapper867 : MapperBase
	{
		/// <summary>
		/// Default constructor
		/// </summary>
		...
		public RgeMapper867( string utilityCode, string marketCode )
		{
		...
		{
			RgeMarker rgeMarker = new RgeMarker();
			...
			foreach( string fc in fileCellList )
			{
				...
				switch( cell0 )
				{
					...
					case "PTD":													// new usage record..
						{
							...
							cellContents = cells[rgeMarker.PtdLoopCell].Trim();
							ptdLoop = cellContents == null ? "" : cellContents;

							// BPT^00 contain mainly PTD^BQ.. also, could not find any BPT^52
							if( ptdLoop == "BQ" )
								ptdLoop = "SU";
					...
					case "BPT": // transaction set purpose code
						{
							cellContents = cells[rgeMarker.TransactionSetPurposeCodeCell].Trim();
							transactionSetPurposeCode = cellContents == null ? "" : cellContents;
                            cellContents = "";
                            if (cells.Length > 3)
                            {
                                string dateString = string.Empty;
                                dateString = cells[rgeMarker.TransactionCreationDateCell].Trim();
				...
				switch( marker )
				{
					case "N18S": // duns number
						{
							cellContents = cells[rgeMarker.DunsNumberCell].Trim();
					...
					case "REFSPL": // zone
						{
							cellContents = cells[rgeMarker.ZoneCell].Trim();
							zone = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFBF":	//bill cycle
						{
							cellContents = cells[rgeMarker.BillGroupCell].Trim();
							billGroup = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFLO":	//load profile
						{
							cellContents = cells[rgeMarker.LoadProfileCell].Trim();
                    ...
                    case "MEAAF":
						{
							...
							if( !fc.Contains( "MU" ) )
							{
								cellContents = cells[rgeMarker.QuantityCell].Trim();
								quantity = cellContents == null ? "" : cellContents;

								cellContents = cells[rgeMarker.UnitOfMeasurementCell].Trim();
								unitOfMeasurement = cellContents == null ? "" : cellContents;

								if( rgeMarker.MeasurementSignificanceCodeCell < cells.Length )
								{
									cellContents = cells[rgeMarker.MeasurementSignificanceCodeCell].Trim();
					...
					case "DTM150": // begin date
						{
							cellContents = cells[rgeMarker.BeginDateCell].Trim();
					...
					case "DTM151": // end date
						{
							cellContents = cells[rgeMarker.EndDateCell].Trim();
					...
					case "QTYQD": // begin usage
						{
							...
							if( !fc.Contains( "NV" ) )
							{
								cellContents = cells[rgeMarker.QuantityAltCell].Trim();
								quantity = cellContents == null ? "" : cellContents;

								cellContents = cells[rgeMarker.UnitOfMeasurementAltCell].Trim();
								unitOfMeasurement = cellContents == null ? "" : cellContents;
							}
							break;
						}
