namespace LibertyPower.Business.MarketManagement.EdiParser.FileParser
{
	...
	/// </summary>
	public class WmecoMapper867 : MapperBase
	{
		/// <summary>
		/// Default constructor
		/// </summary>
		public WmecoMapper867() { }
		...
		public WmecoMapper867( string utilityCode, string marketCode )
		{
		...
		{
			WmecoMarker wmecoMarker = new WmecoMarker();
			...
			foreach( string fc in fileCellList )
			{
				...
				switch( cell0 )
				{
					...
					case "PTD":													// new usage record..
						{
							...
							cellContents = cells[wmecoMarker.PtdLoopCell].Trim();
					...
					case "BPT": // transaction set purpose code
						{
							cellContents = cells[wmecoMarker.TransactionSetPurposeCodeCell].Trim();
                            ...
                            if (cells.Length > 3)
                            {
                                ...
                                dateString = cells[wmecoMarker.TransactionCreationDateCell].Trim();
				...
				switch( marker )
				{
					case "N18S": // duns number
						{
							cellContents = cells[wmecoMarker.DunsNumberCell].Trim();
					...
					case "DTM514": // exchange meter date
						{
							cellContents = cells[wmecoMarker.BeginDateCell].Trim();
					...
					case "DTM151": // end date
						{
							cellContents = cells[wmecoMarker.EndDateCell].Trim();
					...
					case "QTYQD": // begin usage
						{
							...
							if( !fc.Contains( "NV" ) )
							{
								cellContents = cells[wmecoMarker.QuantityAltCell].Trim();
								quantity = cellContents == null ? "" : cellContents;

								cellContents = cells[wmecoMarker.UnitOfMeasurementAltCell].Trim();
								unitOfMeasurement = cellContents == null ? "" : cellContents;
