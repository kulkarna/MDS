...
{
    public class Oncor : IUsageFileParser
    {
        private const string WorksheetName = "Page1_1";
        private DataSet _importFileDataSet;
        public string Error { get; private set; }
        public bool IsParser(UsageFile file)
        {
            return file.UtilityCode.Equals("oncor", StringComparison.InvariantCultureIgnoreCase);
        }
        ...
        public IEnumerable<ParserAccount> Parse(UsageFile file)
        {
            ...
            UtilityDictionary utilities = UtilityService.GetUtilities();
            DataTable dt = _importFileDataSet.Tables[WorksheetName];
            ...
            for (int i = 1; i < dt.Rows.Count; i++)
            {
                if (DataSetService.IsRowEmpty(dt.Rows[i], false) == false)
                {
                    ...
                    if (i == dt.Rows.Count - 1)
                    {
                        ...
                        account = GetUtilityAccountAndUsagesOncor(file.UtilityCode, rows, utilities);
                        if (account != null)
        ...
        private static ParserAccount GetUtilityAccountAndUsagesOncor(string utilityCode, List<DataRow> rows, UtilityDictionary utilities)
        {
