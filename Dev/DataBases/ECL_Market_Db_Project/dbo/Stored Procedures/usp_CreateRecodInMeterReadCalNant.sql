 ...
 * PROCEDURE:	[usp_CreateRecodInMeterReadCalNant]
 ...
 *******************************************************************************
  */
CREATE PROCEDURE usp_CreateRecodInMeterReadCalNant
...
t1.[Read Cycle],
t1.[Read Date]
from [Staging].[Nantucket] t1 with(nolock) 
inner join LibertyPower.[dbo].Utility t2 With(Nolock)
on Left(Upper(t1.Utility),4)=t2.UtilityCode and t1.[Read Cycle] !> 20
and NOT EXISTS
    (SELECT 1 
