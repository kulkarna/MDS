...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[usp_pricing_requests_sel]
...
WHERE	(STATUS <> ''Offer Contracted'')AND(STATUS <> ''Offer Rejected By Customer'')AND(STATUS <> ''Offer Accepted By Customer'')
'

IF LEN(@p_request_id) > 0 
	BEGIN
		SET	@w_sql = @w_sql + ' AND REQUEST_ID = ''' + @p_request_id + ''''
	END

SET	@w_sql = @w_sql + @p_sort_expression
