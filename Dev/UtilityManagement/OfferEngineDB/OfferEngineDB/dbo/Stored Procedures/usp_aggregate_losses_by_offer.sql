...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[usp_aggregate_losses_by_offer]
...
IF EXISTS (	SELECT	NULL
	...
	BEGIN
		SELECT	OFFER_ID, UTILITY, ZONE, LOSSES
		FROM	OE_OFFER_AGGREGATES WITH (NOLOCK)
		WHERE	offer_id = @p_offer_id
	END
ELSE
	BEGIN
		SELECT		offer_id, utility, zone, SUM(Losses*Annual_Usage)/SUM(Annual_Usage) AS Losses
		FROM		(	SELECT	ac.utility,ac.voltage,ac.zone,ac.losses,ac.annual_usage,sap.offer_id 
						FROM	oe_account ac WITH (NOLOCK) INNER JOIN (	SELECT	oa.ACCOUNT_NUMBER, oa.offer_id, a.UTILITY  
																			...
																			AND	oa.ACCOUNT_NUMBER NOT IN -- Do not price zero usage accounts
																			(
