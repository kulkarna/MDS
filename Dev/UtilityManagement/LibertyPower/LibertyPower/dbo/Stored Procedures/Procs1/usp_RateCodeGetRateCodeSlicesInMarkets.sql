 ...
 *
 ********************************************************************************/
CREATE PROCEDURE [dbo].[usp_RateCodeGetRateCodeSlicesInMarkets]
AS
SELECT DISTINCT
...
WHERE
    billing_type <> 'RR' and retail_mkt_id  IN ( SELECT DISTINCT c.retail_mkt_id from lp_common..common_utility as c WHERE c.billing_type = 'RR')
ORDER BY
	Market,
    Utility,
    ServiceClass,
...
GO
EXECUTE sp_addextendedproperty @name = N'VirtualFolder_Path', @value = N'RateCode', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_RateCodeGetRateCodeSlicesInMarkets';
