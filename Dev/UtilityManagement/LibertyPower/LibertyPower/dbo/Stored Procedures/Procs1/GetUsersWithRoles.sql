...
-- =============================================
/*
GetUsersWithRoles 'Administrators,Registered Users,Subscribers', 1
*/
...
SELECT DISTINCT
		...
		IsActive  
From [User] (NOLOCK)
JOIN userRole (NOLOCK) ON userRole.UserID = [user].UserID
JOIN [role] (NOLOCK) ON [role].RoleID = userRole.RoleID
...
JOIN (SELECT U.[UserID], COUNT(1) AS [COUNT] FROM [User] U 
	JOIN userRole (NOLOCK) ON userRole.UserID = U.UserID
	JOIN [role] (NOLOCK) ON [role].RoleID = userRole.RoleID
	WHERE [role].RoleName IN (SELECT * FROM @roles)
