...
begin tran
exec libertypower..usp_DevApplicationsTest_v2 @p_TestDealEntry  = 1, @p_TestDealScreening = 0,
@p_TestChangeOfOwnership = 1, @p_TestRenewal = 0
...
begin tran
exec libertypower..usp_DevApplicationsTest_v2 @p_TestDealEntry  = 1, @p_TestDealScreening = 0,
@p_TestChangeOfOwnership = 0, @p_TestRenewal = 0

rollback

SELECT * from LP_ACCOUNT..account WHERE contract_nbr = 'TEST1'
...
rollback
*/
CREATE PROC [dbo].[usp_DevApplicationsTest_v2](
...
BEGIN
	...
	DECLARE @w_current_date DATETIME
	SET @w_current_date = getdate()
	
	--Contract eff start date will be the first day of next month
	DECLARE @w_contract_eff_start_date DATETIME
    SET @w_contract_eff_start_date = CAST(FLOOR(CAST(dateadd(mm,datediff(mm,0,dateadd(mm, 1,@w_current_date)),0) AS float)) AS datetime)
    
    DECLARE @w_contract_flow_date DATETIME
    IF datepart(dw, @w_contract_eff_start_date) = 1
	BEGIN
		--If contract eff start date is a sunday, adds 1 day
		SET @w_contract_flow_date = DATEADD(dd, 1, @w_contract_eff_start_date)
	END
	ELSE IF datepart(dw, @w_contract_eff_start_date)= 7
	BEGIN
		--If contract eff start date is a saturday, adds 2 days
		SET @w_contract_flow_date = DATEADD(dd, 2, @w_contract_eff_start_date)
	END
	ELSE
	BEGIN
		SET @w_contract_flow_date = @w_contract_eff_start_date
	END
    
    --Deal date will be today's date
    DECLARE @w_deal_date DATETIME
    SET @w_deal_date = CAST(FLOOR(CAST(@w_current_date AS float)) AS datetime)
    
    --Get rate and terms for a specific product and today's date
    DECLARE @w_term_months INT
    DECLARE @w_rate_id INT
	DECLARE @w_rate FLOAT
	
	SELECT TOP 1 @w_term_months = h.term_months,
				 @w_rate_id		= r.rate_id, 
				 @w_rate		= r.rate
	FROM lp_common..product_rate_history h (NOLOCK)
	JOIN lp_common..common_product		 p (NOLOCK) ON h.product_id = p.product_id
	JOIN lp_common..common_product_rate  r (NOLOCK) ON r.product_id = p.product_id
												   AND h.rate_id = r.rate_id
	WHERE h.eff_date = @w_deal_date
	 AND  p.product_id = 'ONCOR_FS_ABC'
	 AND  h.contract_eff_start_date = @w_contract_eff_start_date
	 
	--Date end = eff start date + terms - 1 day
	DECLARE @w_end_date DATETIME
    SET @w_end_date = DATEADD(mm, @w_term_months, DATEADD(dd, -1, @w_contract_eff_start_date))
	...
	BEGIN
		...
		exec lp_deal_capture..usp_contract_pricing_ins @p_username=@p_UsernameDefault,@p_contract_nbr=@p_ContractNumber,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_account_type=N'1',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_contract_eff_start_date=@w_contract_eff_start_date,@p_enrollment_type=N'1',@p_requested_flow_start_date=@w_contract_flow_date,@p_term_months=@w_term_months,@p_account_number=N'CONTRACT',@p_customer_code=N'',@p_customer_group=N'',@p_transfer_rate=@w_rate,@p_HeatIndexSourceID=default,@p_HeatRate=default,@p_comm_cap=N'0.02',@p_contract_date=@w_deal_date
		...
		set @w_current_date = getdate()
		exec lp_account..usp_account_status_history_ins @p_username=@p_UsernameDefault,@p_account_id=@p_AccountId2,@p_status=N'03000',@p_sub_status=N'10',@p_date_created=@w_current_date,@p_process_id=N'DEAL CAPTURE',@p_param_01=N'ONCOR',@p_param_02=N'',@p_param_03=N'',@p_param_04=N'',@p_param_05=N'',@p_param_06=N'',@p_param_07=N'',@p_param_08=N'',@p_date_eff=@w_current_date,@p_result_ind=N'N'
		...
		@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,
		...
		@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'',@p_contract_eff_start_date=@w_contract_eff_start_date,
		@p_term_months=@w_term_months,@p_date_end=@w_end_date,@p_date_deal=@w_deal_date,
		@p_date_created=@w_current_date,@p_date_submit=@w_current_date,
		@p_sales_channel_role=N'Sales Channel/outboundtelesales',@p_sales_rep=N'AL TAFUR',@p_origin=N'ONLINE',
		@p_annual_usage=0,@p_date_flow_start='Jan  1 1900 12:00:00:000AM',@p_date_por_enrollment=@w_deal_date,
		...
		@p_loss_code=N'',@p_meter_type=N'',@p_requested_flow_start_date=@w_contract_flow_date,@p_deal_type=N'',
		...
		set @w_current_date = getdate()
		exec lp_account..usp_account_status_history_ins @p_username=@p_UsernameDefault,@p_account_id=@p_AccountId1,@p_status=N'03000',@p_sub_status=N'10',@p_date_created=@w_current_date,@p_process_id=N'DEAL CAPTURE',@p_param_01=N'ONCOR',@p_param_02=N'',@p_param_03=N'',@p_param_04=N'',@p_param_05=N'',@p_param_06=N'',@p_param_07=N'',@p_param_08=N'',@p_date_eff=@w_current_date,@p_result_ind=N'N'
		...
		@p_contract_type=N'VOICE',@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,
		...
		@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,
		@p_date_end=@w_end_date,@p_date_deal=@w_deal_date,@p_date_created=@w_current_date,
		@p_date_submit=@w_current_date,@p_sales_channel_role=N'Sales Channel/outboundtelesales',@p_sales_rep=N'AL TAFUR',
		@p_origin=N'ONLINE',@p_annual_usage=0,@p_date_flow_start='Jan  1 1900 12:00:00:000AM',@p_date_por_enrollment=@w_deal_date,
		...
		@p_requested_flow_start_date=@w_contract_flow_date,@p_deal_type=N'',@p_enrollment_type=1,@p_customer_code=N'',@p_customer_group=N'',
		...
		@p_check_type=N'TPV',@p_check_request_id=N'ENROLLMENT',@p_approval_status=N'PENDING',@p_approval_status_date=@w_current_date,
		...
		--exec LibertyPower..usp_AccountEventHistoryInsert @ContractNumber=@p_ContractNumber,@AccountId=@p_AccountId1,@ProductId=N'ONCOR_FS_ABC',@RateID=@w_rate_id,@Rate=@w_rate,@RateEndDate=@w_end_date,@EventID=1,@EventEffectiveDate=@w_contract_eff_start_date,@ContractType=N'VOICE',@ContractDate=@w_deal_date,@ContractEndDate=@w_end_date,@DateFlowStart='Jan  1 1900 12:00:00:000AM',@Term=@w_term_months,@AnnualUsage=25000,@GrossMarginValue=6.500000,@AnnualGrossProfit=162.500000,@TermGrossProfit=40.62500000,@AnnualGrossProfitAdj=0,@TermGrossProfitAdj=0,@AnnualRevenue=1242.75000,@TermRevenue=310.6875000,@AnnualRevenueAdj=0,@TermRevenueAdj=0,@AdditionalGrossMargin=0,@SubmitDate=getdate(),@DealDate=@w_deal_date,@SalesChannelId=N'SALES CHANNEL/OUTBOUNDTELESALES',@SalesRep=N'AL TAFUR'
	...
	BEGIN
		...
		exec lp_enrollment..usp_check_account_approval_reject @p_username=@p_UsernameDefault,@p_check_request_id=N'ENROLLMENT',@p_contract_nbr=@p_ContractNumber,@p_account_id=N'NONE',@p_account_number=N'',@p_check_type=N'TPV',@p_approval_status=N'APPROVED',@p_comment=N'test'
			...
			set @w_current_date = getdate()
			exec LibertyPower..usp_CreditScoreHistoryInsert @p_CustomerName=NULL,@p_StreetAddress=NULL,@p_City=NULL,@p_State=NULL,@p_ZipCode=NULL,@p_DateAcquired=@w_current_date,@p_AgencyReferenceID=0,@p_CreditAgencyID=3,@p_Score=85,@p_ScoreType=N'Commercial Service',@p_FullXMLReport=NULL,@p_Source=N'Manual',@p_CustomerID=0,@p_Contract_nbr=@p_ContractNumber,@p_Account_nbr=N'',@p_Username=@p_UsernameDefault,@p_DateCreated=@w_current_date
			...
			--exec LibertyPower..usp_AccountEventHistoryInsert @ContractNumber=@p_ContractNumber,@AccountId=@p_AccountId1,@ProductId=N'ONCOR_FS_ABC',@RateID=@w_rate_id,@Rate=@w_rate,@RateEndDate='Jan  1 1900 12:00:00:000AM',@EventID=7,@EventEffectiveDate=getdate(),@ContractType=N'VOICE',@ContractDate=@w_deal_date,@ContractEndDate=@w_end_date,@DateFlowStart='Jan  1 1900 12:00:00:000AM',@Term=@w_term_months,@AnnualUsage=25000,@GrossMarginValue=6.500000,@AnnualGrossProfit=0,@TermGrossProfit=0,@AnnualGrossProfitAdj=0,@TermGrossProfitAdj=40.6250,@AnnualRevenue=0,@TermRevenue=0,@AnnualRevenueAdj=0,@TermRevenueAdj=310.6875,@AdditionalGrossMargin=0,@SubmitDate=getdate(),@DealDate=@w_deal_date,@SalesChannelId=N'SALES CHANNEL/OUTBOUNDTELESALES',@SalesRep=N'AL TAFUR'
			--exec LibertyPower..usp_AccountEventHistoryInsert @ContractNumber=@p_ContractNumber,@AccountId=@p_AccountId2,@ProductId=N'ONCOR_FS_ABC',@RateID=@w_rate_id,@Rate=@w_rate,@RateEndDate='Jan  1 1900 12:00:00:000AM',@EventID=7,@EventEffectiveDate=getdate(),@ContractType=N'VOICE',@ContractDate=@w_deal_date,@ContractEndDate=@w_end_date,@DateFlowStart='Jan  1 1900 12:00:00:000AM',@Term=@w_term_months,@AnnualUsage=25000,@GrossMarginValue=6.500000,@AnnualGrossProfit=0,@TermGrossProfit=0,@AnnualGrossProfitAdj=0,@TermGrossProfitAdj=40.6250,@AnnualRevenue=0,@TermRevenue=0,@AnnualRevenueAdj=0,@TermRevenueAdj=310.6875,@AdditionalGrossMargin=0,@SubmitDate=getdate(),@DealDate=@w_deal_date,@SalesChannelId=N'SALES CHANNEL/OUTBOUNDTELESALES',@SalesRep=N'AL TAFUR'
	...
	BEGIN
		...
		--Insert contract into renewal tables
		...
		--Insert addresses, contacts and names
		...
		set @w_current_date = getdate()
		exec lp_contract_renewal..usp_contract_transfer_contract_upd @p_username=@p_UsernameDefault,@p_original_contract_nbr=@p_ContractNumber,@p_contract_nbr=@p_ContractNumberOT,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_customer_name_link =1,@p_customer_address_link=1,@p_customer_contact_link=1,@p_billing_address_link=1,@p_billing_contact_link=1,@p_owner_name_link=1,@p_service_address_link=5,@p_business_type=N'CORPORATION',@p_business_activity=N'CORPORATION',@p_additional_id_nbr=N'',@p_date_end=@w_end_date,@p_term_months=@w_term_months,@p_sales_channel_role=N'SALES CHANNEL/OUTBOUNDTELESALES',@p_sales_rep=N'AL TAFUR',@p_date_submit=@w_current_date
		
		--Insert account 1
		exec lp_contract_renewal..usp_contract_transfer_account_ins @p_contract_nbr=@p_ContractNumberOT,@p_contract_type=N'VOICE',@p_account_number=@p_AccountNumber2,@p_status=N'905000',@p_account_id=@p_AccountId2,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_account_name_link=1,@p_customer_name_link =1,@p_customer_address_link=1,@p_customer_contact_link=1,@p_billing_address_link=1,@p_billing_contact_link=1,@p_owner_name_link=1,@p_service_address_link=5,@p_business_type=N'CORPORATION',@p_business_activity=N'CORPORATION',@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,@p_date_end=@w_end_date,@p_date_deal=@w_deal_date,@p_date_created=@w_current_date,@p_date_submit=@w_current_date,@p_sales_channel_role=N'SALES CHANNEL/OUTBOUNDTELESALES',@p_username=@p_UsernameDefault,@p_sales_rep=N'AL TAFUR',@p_origin=N'ONLINE',@p_annual_usage=3252,@p_chgstamp=0

		--Insert account 2
		exec lp_contract_renewal..usp_contract_transfer_account_ins @p_contract_nbr=@p_ContractNumberOT,@p_contract_type=N'VOICE',@p_account_number=@p_AccountNumber1,@p_status=N'905000',@p_account_id=@p_AccountId1,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_account_name_link=1,@p_customer_name_link =1,@p_customer_address_link=1,@p_customer_contact_link=1,@p_billing_address_link=1,@p_billing_contact_link=1,@p_owner_name_link=1,@p_service_address_link=2,@p_business_type=N'CORPORATION',@p_business_activity=N'CORPORATION',@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,@p_date_end=@w_end_date,@p_date_deal=@w_deal_date,@p_date_created=@w_current_date,@p_date_submit=@w_current_date,@p_sales_channel_role=N'SALES CHANNEL/OUTBOUNDTELESALES',@p_username=@p_UsernameDefault,@p_sales_rep=N'AL TAFUR',@p_origin=N'ONLINE',@p_annual_usage=3252,@p_chgstamp=0

		--Save new pricing data for the transfer
		exec lp_contract_renewal..usp_contract_transfer_general_ins @p_username=@p_UsernameDefault,@p_contract_nbr=@p_ContractNumberOT,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_business_type=N'CORPORATION',@p_business_activity=N'NONE',@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,@p_sales_channel_role=N'Sales Channel/outboundtelesales',@p_sales_rep=N'AL TAFUR',@p_account_number=@p_AccountNumber1,@p_date_submit=@w_current_date,@p_error=N'',@p_msg_id=N'',@p_descp=N'',@p_result_ind=N'Y',@p_sales_mgr=N'Dennis Manning',@p_initial_pymt_option_id=NULL,@p_residual_option_id=N'2',@p_evergreen_option_id=N'5',@p_evergreen_commission_rate=N'0'

		exec lp_contract_renewal..usp_contract_transfer_general_ins @p_username=@p_UsernameDefault,@p_contract_nbr=@p_ContractNumberOT,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_business_type=N'CORPORATION',@p_business_activity=N'NONE',@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,@p_sales_channel_role=N'Sales Channel/outboundtelesales',@p_sales_rep=N'AL TAFUR',@p_account_number=@p_AccountNumber2,@p_date_submit=@w_current_date,@p_error=N'',@p_msg_id=N'',@p_descp=N'',@p_result_ind=N'Y',@p_sales_mgr=N'Dennis Manning',@p_initial_pymt_option_id=NULL,@p_residual_option_id=N'2',@p_evergreen_option_id=N'5',@p_evergreen_commission_rate=N'0'
	
		--Add/Save addresses, contacts and names
	...
	--Check if a transfer of ownership was performed and change base contract number
	...
	BEGIN
		...
		set @w_current_date = getdate()
		exec lp_contract_renewal..usp_contract_renewal_contract_upd @p_username=@p_UsernameDefault,@p_contract_nbr=@p_ContractNumberR,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_customer_name_link =2,@p_customer_address_link=3,@p_customer_contact_link=2,@p_billing_address_link=3,@p_billing_contact_link=2,@p_owner_name_link=2,@p_service_address_link=3,@p_business_type=N'CORPORATION',@p_business_activity=N'NONE',@p_additional_id_nbr=N'NONE',@p_additional_id_nbr_type=N'NONE',@p_date_end=@w_end_date,@p_term_months=@w_term_months,@p_sales_channel_role=N'SALES CHANNEL/OUTBOUNDTELESALES',@p_sales_rep=N'AL TAFUR',@p_date_submit=@w_current_date,@p_SSNClear=NULL,@p_SSNEncrypted=NULL,@p_CreditScoreEncrypted=N'Pd/FDEbCpPN8mDTJ5zc5mw==',@p_contract_eff_start_date=@w_contract_eff_start_date
		
		--Insert account 1
		exec lp_contract_renewal..usp_contract_renewal_account_ins @p_contract_nbr=@p_ContractNumberR,@p_contract_type=N'VOICE RENEWAL',@p_account_number=@p_AccountNumber1,@p_status=N'905000',@p_account_id=@p_AccountId1,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_account_name_link=2,@p_customer_name_link =2,@p_customer_address_link=3,@p_customer_contact_link=2,@p_billing_address_link=3,@p_billing_contact_link=2,@p_owner_name_link=2,@p_service_address_link=3,@p_business_type=N'CORPORATION',@p_business_activity=N'NONE',@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'NONE',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,@p_date_end=@w_end_date,@p_date_deal=@w_deal_date,@p_date_created=@w_current_date,@p_date_submit=@w_current_date,@p_sales_channel_role=N'SALES CHANNEL/OUTBOUNDTELESALES',@p_username=@p_UsernameDefault,@p_sales_rep=N'AL TA',@p_sales_mgr=N'Dennis Manning',@p_initial_pymt_option_id=2,@p_residual_option_id=5,@p_evergreen_option_id=0,@p_residual_commission_end=NULL,@p_evergreen_commission_end=NULL,@p_evergreen_commission_rate=0,@p_origin=N'ONLINE',@p_annual_usage=3252,@p_chgstamp=0,@p_SSNClear=NULL,@p_SSNEncrypted=NULL,@p_CreditScoreEncrypted=N'Pd/FDEbCpPN8mDTJ5zc5mw==',@p_account_eff_start_date=@w_contract_eff_start_date

		--Insert account 2
		exec lp_contract_renewal..usp_contract_renewal_account_ins @p_contract_nbr=@p_ContractNumberR,@p_contract_type=N'VOICE RENEWAL',@p_account_number=@p_AccountNumber2,@p_status=N'905000',@p_account_id=@p_AccountId2,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_account_name_link=2,@p_customer_name_link =2,@p_customer_address_link=6,@p_customer_contact_link=2,@p_billing_address_link=6,@p_billing_contact_link=2,@p_owner_name_link=2,@p_service_address_link=6,@p_business_type=N'CORPORATION',@p_business_activity=N'NONE',@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'NONE',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,@p_date_end=@w_end_date,@p_date_deal=@w_deal_date,@p_date_created=@w_current_date,@p_date_submit=@w_current_date,@p_sales_channel_role=N'SALES CHANNEL/OUTBOUNDTELESALES',@p_username=@p_UsernameDefault,@p_sales_rep=N'AL TAFUR',@p_sales_mgr=N'Dennis Manning',@p_initial_pymt_option_id=2,@p_residual_option_id=5,@p_evergreen_option_id=0,@p_residual_commission_end=NULL,@p_evergreen_commission_end=NULL,@p_evergreen_commission_rate=0,@p_origin=N'ONLINE',@p_annual_usage=3252,@p_chgstamp=0,@p_SSNClear=NULL,@p_SSNEncrypted=NULL,@p_CreditScoreEncrypted=N'Pd/FDEbCpPN8mDTJ5zc5mw==',@p_account_eff_start_date=@w_contract_eff_start_date

		--Insert add on account
		...
		--Set dates and rate properly to the renewal contract:
		--Get new effective start date based on the previous contract date end
		SET @w_contract_eff_start_date = DATEADD(dd, 1, (select top 1 date_end from lp_account..account where contract_nbr = @p_ContractNumber))
		
		
		--Get rate for the new eff start date	
		SELECT TOP 1 @w_term_months = h.term_months,
					 @w_rate_id		= r.rate_id, 
					 @w_rate		= r.rate
		FROM lp_common..product_rate_history h (NOLOCK)
		...
		--Save new pricing information
		set @w_current_date = getdate()
		exec lp_contract_renewal..usp_contract_general_ins @p_username=@p_UsernameDefault,@p_contract_nbr=@p_ContractNumberR,@p_retail_mkt_id=N'TX',@p_utility_id=N'ONCOR',@p_account_type=N'1',@p_product_id=N'ONCOR_FS_ABC',@p_rate_id=@w_rate_id,@p_rate=@w_rate,@p_business_type=N'CORPORATION',@p_business_activity=N'NONE',@p_additional_id_nbr_type=N'NONE',@p_additional_id_nbr=N'',@p_ssnClear=N'',@p_ssnEncrypted=N'',@p_contract_eff_start_date=@w_contract_eff_start_date,@p_term_months=@w_term_months,@p_sales_channel_role=N'Sales Channel/outboundtelesales',@p_sales_rep=N'Al Tafur',@p_sales_mgr=N'Dennis Manning',@p_initial_pymt_option_id=NULL,@p_residual_option_id=NULL,@p_evergreen_option_id=NULL,@p_residual_commission_end=NULL,@p_evergreen_commission_end=NULL,@p_evergreen_commission_rate=NULL,@p_account_number=N'CONTRACT',@p_date_submit=@w_current_date,@p_ContractDate=@w_deal_date,@p_error=N'',@p_msg_id=N'',@p_descp=N'',@p_result_ind=N'Y'

		--Add/Save addresses, names and contacts
		...
		set @w_current_date = getdate()
		exec lp_contract_renewal..usp_renewal_detail_upd @p_username=@p_UsernameDefault,@p_contract_nbr=@p_ContractNumber,@p_call_status=N'S',@p_call_reason_code=N'1023',@p_assigned_to_username=@p_UsernameDefault,@p_action_type=N'NONE',@p_date_resolved=@w_current_date,@p_nextcalldate=N'',@p_comments=N'Contract renewed',@p_old_chgstamp=N'0',@p_reason_code_sub_array=N''
