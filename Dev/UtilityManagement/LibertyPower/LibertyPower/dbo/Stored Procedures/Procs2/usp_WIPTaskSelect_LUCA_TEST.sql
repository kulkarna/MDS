...
exec lp_enrollment..usp_check_account_sel_list_eric @p_username=N'LIBERTYPOWER\dmarino',@p_view=N'ALL',@p_rec_sel=N'50',@p_contract_nbr_filter=N'02795488',@p_account_id_filter=N'NONE',@p_account_number_filter=N'NONE',@p_check_type_filter=N'ALL',@p_check_r
equest_id_filter=N'NONE'   
  
*/   
  
CREATE PROCEDURE [dbo].[usp_WIPTaskSelect_LUCA_TEST]  
		...
		ELSE  
			IF @TaskTypeID IS NOT NULL
			 BEGIN
				 ...
				 INSERT INTO #WIP  
				 SELECT TOP (@p_rec_sel) WT.WIPTaskHeaderID  
				 FROM LibertyPower..WIPTask   WT  (NOLOCK)  
				 JOIN LibertyPower..WorkflowTask     WFT (NOLOCK) ON WT.workflowtaskid = WFT.workflowtaskid  
				 WHERE WFT.TaskTypeID = @TaskTypeID  
				  
				 Create clustered index cidx1 on #WIP(WIPTaskHeaderID)
				 
				 INSERT INTO #ContractTemp  
				 SELECT TOP (@p_rec_sel) WTH.ContractID  
				 FROM LibertyPower..WIPTaskHeader WTH (NOLOCK)     
				 JOIN #WIP W ON WTH.WIPTaskHeaderID = W.WIPTaskHeaderID  
			...
			ELSE
				BEGIN  
				 INSERT INTO #ContractTemp  
				 SELECT TOP (@p_rec_sel) ContractID  
				 FROM LibertyPower.dbo.Contract (NOLOCK)  
				END  
  
  
Create clustered index cidx1 on #ContractTemp(ContractID)
