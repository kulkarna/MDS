...
CREATE PROCEDURE [dbo].[usp_VRE_UCapFactorCurveSelect]	
	@BeginDate		DATETIME,
	@EndDate		DATETIME,
...
BEGIN
	...
	BEGIN
		SELECT	c.ID, c.ZoneId, c.[Month], c.[Year], c.UCapFactor,
			c.FileContextGuid, c.DateCreated, c.CreatedBy
	...
	BEGIN
		...
		WHERE c.ID IN
				(SELECT MAX(UC.ID)
				 FROM VREUCapFactorCurve UC WITH (NOLOCK)			
...
GO
EXECUTE sp_addextendedproperty @name = N'VirtualFolder_Path', @value = N'VRE', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_VRE_UCapFactorCurveSelect';
