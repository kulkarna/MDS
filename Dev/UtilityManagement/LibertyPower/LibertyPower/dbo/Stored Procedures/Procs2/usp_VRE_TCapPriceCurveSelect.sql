...
CREATE PROCEDURE [dbo].[usp_VRE_TCapPriceCurveSelect]	
	@BeginDate		DATETIME,
	@EndDate		DATETIME,
...
BEGIN
	...
	BEGIN
		SELECT	c.ID, c.ZoneId, c.[Month], c.[Year], c.TCapPrice, c.CreatedBy , c.FileContextGuid, c.DateCreated		
	...
	BEGIN
		...
		WHERE c.ID IN
			(SELECT MAX(TC.ID)
			 FROM VRETCapPriceCurve TC WITH (NOLOCK)			
...
GO
EXECUTE sp_addextendedproperty @name = N'VirtualFolder_Path', @value = N'VRE', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_VRE_TCapPriceCurveSelect';
