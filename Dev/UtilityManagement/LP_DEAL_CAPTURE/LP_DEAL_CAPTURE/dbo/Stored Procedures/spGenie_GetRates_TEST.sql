...
--exec [spGenie_GetRates_TEST]


CREATE proc [dbo].[spGenie_GetRates_TEST](@ExpirationFlag int=0) as
begin
...
DECLARE	@ProductCrossPriceSetID	int,
	...
	WHILE (SELECT COUNT(PriceID) FROM @PriceIDs) > 0
		BEGIN
			SELECT TOP 1 @PriceID = PriceID FROM @PriceIDs			
			...
			DELETE FROM @PriceIDs WHERE PriceID = @PriceID
		END	
