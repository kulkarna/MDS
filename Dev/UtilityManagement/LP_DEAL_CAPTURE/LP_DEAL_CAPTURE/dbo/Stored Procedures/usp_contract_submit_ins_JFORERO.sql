...
CREATE procedure [dbo].[usp_contract_submit_ins_JFORERO]
...
begin 
	...
	if exists(select *
									from lp_documents..document_history
	...
	begin tran val
		...
		begin
			...
			begin
				...
				select @w_descp_add                        = ''
				goto goto_select
			end -- END if @@error <> 0  or @@rowcount = 0 

			-- get contract type of contract to be amended
			select top 1  @w_contract_type_amend          = contract_type
			...
			begin
				if (select top 1 ltrim(rtrim(por_option)) from lp_account..account
		...
		end -- END OF if exists(select a.status
		...
		end -- END OF if @w_add_info = 0

		if not exists(select account_id  from lp_account..account_name with (NOLOCK)
						where account_id                   = @w_account_id
						and   name_link                    = @w_account_name_link)
		begin

			insert into lp_account..account_name
			select @w_account_id, @w_account_name_link, LTRIM(RTRIM(full_name)), 0	-- TICKET 20906
			from deal_name
			where contract_nbr                            = @p_contract_nbr
			and   name_link                               = @w_account_name_link
			...
			begin
				...
				exec usp_contract_error_ins 'ENROLLMENT',
					...
					@w_descp_add
				goto goto_select
			end -- END OF if @@error <> 0 or @@rowcount = 0
		end -- END OF if not exists(select account_id  from lp_account..account_name with (NOLOCK)
		...
		begin
			insert into lp_account..account_name
			select @w_account_id, @w_customer_name_link, LTRIM(RTRIM(full_name)),0	-- TICKET 20906
			from deal_name
			where contract_nbr                            = @p_contract_nbr
			and   name_link                               = @w_customer_name_link
		...
		end -- END if not exists(select account_id from lp_account..account_name with (NOLOCK)
		...
		begin
			...
			insert into lp_account..account_name
			select @w_account_id, @w_owner_name_link, LTRIM(RTRIM(full_name)),0		-- TICKET 20906
			from deal_name
			where contract_nbr                            = @p_contract_nbr
			and   name_link                               = @w_owner_name_link
		...
		if not exists(select account_id from lp_account..account_address with (NOLOCK)
						where account_id                   = @w_account_id
						and   address_link                 = @w_customer_address_link)
		begin
			insert into lp_account..account_address
			select @w_account_id, @w_customer_address_link, address,suite,city,state,zip,county,state_fips,county_fips,0
			from deal_address
			where contract_nbr                            = @p_contract_nbr
			and   address_link                            = @w_customer_address_link
		...
		end -- if not exists(select account_id from lp_account..account_address with (NOLOCK)
		...
		begin
			insert into lp_account..account_address
			select @w_account_id, @w_billing_address_link, address,suite,city, state,zip,county,state_fips,county_fips,0
			from deal_address
			where contract_nbr                            = @p_contract_nbr
			and   address_link                            = @w_billing_address_link
		...
		end -- END if not exists(select account_id from lp_account..account_address with (NOLOCK)

		print 'Starting to insert contacts from usp_contract_submit_ins'

		if not exists(select account_id from lp_account..account_contact with (NOLOCK)
						where account_id                   = @w_account_id
						and   contact_link                 = @w_customer_contact_link)
		begin
			insert into lp_account..account_contact
			select	@w_account_id, @w_customer_contact_link,first_name, last_name,title,phone,fax,email,isnull(birthday, '01/01'), 0
			from deal_contact
			where contract_nbr                            = @p_contract_nbr
			and   contact_link                            = @w_customer_contact_link
		...
		end -- END if not exists(select account_id from lp_account..account_contact with (NOLOCK)
		...
		begin
			insert into lp_account..account_contact
			select @w_account_id, @w_billing_contact_link,first_name,last_name,title, phone,fax,email,isnull(birthday, '01/01'), 0
			from deal_contact
			where contract_nbr                            = @p_contract_nbr
			and   contact_link                            = @w_billing_contact_link
		...
		end  -- if not exists(select account_id from lp_account..account_contact with (NOLOCK)
...
--=================================================== LEFT HERE
   ...
   select account_id, 
          meter_number
   from lp_deal_capture..account_meters
   where account_id                                 = @w_account_id
