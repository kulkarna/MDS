...
--exec usp_import_account_ACCOUNT

CREATE procedure [dbo].[usp_import_account_ACCOUNT]
as
...
UPDATE Enrollment_Access..tblAccounts set ContractType = 'TPV'
...
order by upper(ContractType) desc, AccountNumber
   ...
   --begin tran
      ...
      select @w_product_id                             = case when @w_utility_id = 'CONED' and @w_ProductId='GS'
															  then 'CONED-GS-24'
															  when @w_utility_id = 'CONED' and @w_ProductId='NGS'
															  then 'CONED-NGS-24'
															  when @w_utility_id = 'CONED' and @w_ProductId='FXD'
															  then 'CONED-NGS-24'		
	  ...
	  select @w_por_option                             = case when @w_utility_id = 'CONED' 
																AND @w_account_type = 'SMB'                                                             
                                                             then 'YES'
                                                             else 'NO'
                                                         end --Modified by Douglas Marino 06/25/2006
      select @w_billing_type                          = case when @w_utility_id = 'CONED' 
																AND @w_account_type = 'SMB'                                                             
                                                             then 'RR'
														when @w_utility_id = 'BGE' or @w_utility_id = 'PEPCO-MD'
															 then 'BR'
                                                  else 'DUAL'
                                                  end --Modified by Douglas Marino 06/25/2006
