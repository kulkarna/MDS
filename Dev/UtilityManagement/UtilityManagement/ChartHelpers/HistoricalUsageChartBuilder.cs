...
{
    public class HistoricalUsageChartBuilder : ChartBuilder
    {
        public HistoricalUsageChartBuilder(Chart chart) : base(chart, 1) { }
        ...
        protected override void CustomizeChartSeries(IList<Series> seriesList)
        {
            ...
            {
                var salesResults = context.usp_Chart_HuUtilitySla();
                foreach (var result in salesResults)
                    mySeries.Points.AddXY(result.UtilityCode, result.UtilitySla);
            }
        }
        ...
        protected override void CustomizeChartSeriesPorAvgFlatFee(IList<Series> seriesList)
        {
            ...
            using (var context = new DataAccessLayerEntityFramework.Lp_UtilityManagementEntities())
            {
                var salesResults = context.usp_Chart_HuLpSla();
                foreach (var result in salesResults)
                    mySeries.Points.AddXY(result.UtilityCode, result.LpSla);
            }
        }
