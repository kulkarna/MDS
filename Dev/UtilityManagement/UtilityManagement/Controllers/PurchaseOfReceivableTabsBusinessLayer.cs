...
{
    public class PurchaseOfReceivableTabsBusinessLayer : TabsBusinessLayer
    {
        #region private variables
        private string CLASS = "PurchaseOfReceivableTabsBusinessLayer";
        #endregion

        #region public constructors
        public PurchaseOfReceivableTabsBusinessLayer(string messageId, IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger, DataSet dataSet)
            : base(messageId, dataRepository, excelWorksheetUtility, logger, dataSet)
        {
            string method = "PurchaseOfReceivableTabsBusinessLayer(messageId,dataRepository,excelWorksheetUtility,logger,dataSet)";
            try
            {
                ...
                if (_dataSet != null && _dataSet.Tables != null && _dataSet.Tables.Count > 0 && _dataSet.Tables[0] != null && _dataSet.Tables[0].Rows != null)
                {
                    foreach (DataTable dataTable in _dataSet.Tables)
                    {
                        if (dataTable != null && !string.IsNullOrWhiteSpace(dataTable.TableName))
                        {
                            ...
                            {
                                case "Purchase Of Receivable":
                                    ITabBusinessLayer purchaseOfReceivableTabBusinessLayer = new PurchaseOfReceivableTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(purchaseOfReceivableTabBusinessLayer);
                                    break;
                            }
                        }
                    }
