...
namespace UtilityManagement.Controllers
{
    ...
    public class AuditRequestModeHistoricalUsageController : ControllerBaseWithoutUtilDropDown
    {
        ...
        public AuditRequestModeHistoricalUsageController() : base()
        {
            ViewBag.PageName = "AuditRequestModeHistoricalUsage";
            ViewBag.IndexPageName = "AuditRequestModeHistoricalUsage";
            ViewBag.PageDisplayName = "Audit Request Mode Historical Usage";
        }
        #endregion
        ...
        #region actions
        //
        // GET: /AuditRequestModeHistoricalUsageController/
        public ActionResult Index()
        {
            ...
            catch (Exception exc)
            {
                ...
                return View(new List<usp_zAuditRequestModeHistoricalUsage_SELECT1_Result>());
            }
        }
        ...
        //
        // GET: /AuditRequestModeHistoricalUsageController/Details/5
        public ActionResult Details(Guid id)
        {
            usp_zAuditRequestModeHistoricalUsage_SELECT1_Result usp_zAuditRequestModeHistoricalUsage_SELECT_Result = _db.usp_zAuditRequestModeHistoricalUsage_SELECT1().Where(x => x.Id == id).FirstOrDefault();
            if (usp_zAuditRequestModeHistoricalUsage_SELECT_Result == null)
            {
                return HttpNotFound();
            }
            return View(usp_zAuditRequestModeHistoricalUsage_SELECT_Result);
        }
        ...
        {
            var response = _db.usp_zAuditRequestModeHistoricalUsage_SELECT1().OrderBy(x => x.Id).ToList();
            ...
            switch (Session[Common.SORTCOLUMNNAME].ToString())
            {
                ...
                case "UtilitysSlaHistoricalUsageResponseInDays":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.UtilitysSlaHistoricalUsageResponseInDays).ToList();
                    else
                        response = response.OrderBy(x => x.UtilitysSlaHistoricalUsageResponseInDays).ToList();
                    break;
                case "UtilitysSlaHistoricalUsageResponseInDaysPrevious":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.UtilitysSlaHistoricalUsageResponseInDaysPrevious).ToList();
                    else
                        response = response.OrderBy(x => x.UtilitysSlaHistoricalUsageResponseInDaysPrevious).ToList();
                    break;
                case "LibertyPowersSlaFollowUpHistoricalUsageResponseInDays":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.LibertyPowersSlaFollowUpHistoricalUsageResponseInDays).ToList();
                    else
                        response = response.OrderBy(x => x.LibertyPowersSlaFollowUpHistoricalUsageResponseInDays).ToList();
                    break;
                case "LibertyPowersSlaFollowUpHistoricalUsageResponseInDaysPrevious":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.LibertyPowersSlaFollowUpHistoricalUsageResponseInDaysPrevious).ToList();
                    else
                        response = response.OrderBy(x => x.LibertyPowersSlaFollowUpHistoricalUsageResponseInDaysPrevious).ToList();
                    break;
                case "IsLoaRequired":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
        ...
        private List<usp_zAuditRequestModeHistoricalUsage_SELECT1_Result> ObtainResponse()
        {
            var response = _db.usp_zAuditRequestModeHistoricalUsage_SELECT1().OrderBy(x => x.Id).ToList();
            if (Session[Common.SORTCOLUMNNAME] == null)
                Session[Common.SORTCOLUMNNAME] = "Id";
