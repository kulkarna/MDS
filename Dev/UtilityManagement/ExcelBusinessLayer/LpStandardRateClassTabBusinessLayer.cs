...
{
    public class LpStandardRateClassTabBusinessLayer : TabBusinessLayer
    {
        #region private variables
        private string CLASS = "LpStandardRateClassTabBusinessLayer";
        const string LP_STANDARD_RATE_CLASS_CODE = "LpStandardRateClassCode";
        protected const string TAB_NAME = "LP Std Rate Class";
        #endregion

        #region public constructors
        public LpStandardRateClassTabBusinessLayer(string messageId, IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger, DataTable dataTable, List<DataAccessLayerEntityFramework.UtilityCompany> utilityCompanies)
            : base(messageId, dataRepository, excelWorksheetUtility, logger, TAB_NAME, utilityCompanies)
        {
            string method = string.Format("{0}.{1}.LpStandardRateClassTabBusinessLayer(messageId,dataRepository,excelWorksheetUtility,logger,dataTable)", NAMESPACE, CLASS);
            try
            {
                _logger.LogInfo(messageId, string.Format("{0}.{1}.{2} {3}", NAMESPACE, CLASS, method, Common.BEGIN));

                Columns.Add(UTILITY_CODE);
                Columns.Add(LP_STANDARD_RATE_CLASS_CODE);
                    ...
                    foreach (DataRow dataRow in _dataTable.Rows)
                    {
                        if (rowCount < _dataTable.Rows.Count)
                        {
                            ...
                            (
                                ...
                                && Common.NullSafeString(dataRow[LP_STANDARD_RATE_CLASS_CODE]).Length <= 255
                            ...
                            )
                            {
                                ...
                                try
                                {
                                    DataSet dataSet = _dataRepository.usp_LpStandardRateClass_UPSERT(messageId, Guid.NewGuid().ToString(),
                                        Common.NullSafeString(dataRow[UTILITY_CODE]),
                                        Common.NullSafeString(dataRow[LP_STANDARD_RATE_CLASS_CODE]),
                                        inactive,
                                        userName);
        ...
        protected override bool DoesDataTableHaveAllValidColumns(string messageId)
        {
            ...
            try
            {
                ...
                returnValue = _dataTable != null
                    ...
                    && DoesDataTableHaveColumn(messageId, LP_STANDARD_RATE_CLASS_CODE)
