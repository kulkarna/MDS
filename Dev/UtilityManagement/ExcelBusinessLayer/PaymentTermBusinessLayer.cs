...
{
    public class PaymentTermBusinessLayer : SheetBusinessLayer
    {
        #region private variables
        private const string CLASS = "PaymentTermBusinessLayer";
        private const string TABLE_ONE_NAME = "Payment Term";
        #endregion

        #region public constructors
        public PaymentTermBusinessLayer(IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger)
        ...
        protected override DataSet GetTableOneDataSetAll(string messageId)
        {
            ...
            try
            {
                ...
                DataSet dataSet = _repository.usp_PaymentTerm_GetAll(messageId);
        ...
        protected override DataSet GetTableOneDataSet(string messageId, string utilityCode)
        {
            ...
            try
            {
                ...
                DataSet dataSet = _repository.usp_PaymentTerm_GetByUtilityCode(messageId, utilityCode);
                _logger.LogInfo(messageId, string.Format("{0}.{1}.{2} dataSet.Tables[0].TableName = TABLE_ONE_NAME;", NAMESPACE, CLASS, method));
                dataSet.Tables[0].TableName = TABLE_ONE_NAME;
        ...
        internal override void InsertDataTables(string messageId, DataSet dataSet, string userName)
        {
            ...
            try
            {
                ...
                _tabsBusinessLayer = new PaymentTermTabsBusinessLayer(messageId, _repository, _excelWorksheetUtility, _logger, dataSet, userName);
                _logger.LogInfo(messageId, string.Format("{0}.{1}.{2} _tabsBusinessLayer = new PaymentTermTabsBusinessLayer(messageId, _repository, _excelWorksheetUtility, _logger, dataSet, userName:{3})", NAMESPACE, CLASS, method, userName));
