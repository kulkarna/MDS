...
-- SR1-78040921        
-- =======================================        
-- Modified: Rafael Vasques 7/9/2013       
-- Adding Status of Submissionqueue on the tables.      
-- IT 121 Data Quality        
-- ======================================= 
-- Modified: Agata Studzinska 11/20/2013  
-- Any status of the Tax Exemption queues shouldn't affect the Contract Status.  
-- Added filter to update contract status only  IF (@p_WorkflowID <> @TaxExemptWorkflowId)  
-- IT 124   
-- ========================================      
-- Modified: Diogo Lima 12/4/2013       
-- Adding Status of Submissionqueue on the tables for new deals.      
-- IT 121 Data Quality        
-- =======================================  
-- Modified: Agata Studzinska 12/20/2013
-- Added code to ensure once all tasks are approved status of accounts
-- gets updated to 05000/10 or 07000/10
-- IT 124
-- =======================================
...
ALTER PROCEDURE [dbo].[usp_WIPTaskUpdateStatus]        
...
-------------------------------------        
 ...
 BEGIN        
  ...
  BEGIN        
   ...
   BEGIN        
    ...
    IF (@p_WorkflowID <> @TaxExemptWorkflowId) -- IT124 Agata Studzinska 20/11/2013  
    BEGIN  
		UPDATE  [Contract]        
 ...
 --Begin IT 121 Data Quality [Rafael Vasques] 07/09/2013      
   UPDATE ac        
   SET AccountContractStatusReasonID = 2, AccountContractStatusID =3       
   FROM LibertyPower..AccountContract  AC WITH (NOLOCK)         
    ...
    ON U.Id       = A.UtilityId        
    WHERE C.ContractId    = @p_ContractID           
    and ac.AccountContractStatusID not in (2,3)    
         
     --End IT 121 Data Quality [Rafael Vasques] 07/09/2013      
       
     --Begin IT 121 Data Quality [Diogo Lima] 12/4/2013      
    UPDATE ac        
    SET AccountContractStatusReasonID = 2, AccountContractStatusID =3       
    FROM LibertyPower..AccountContract  AC WITH (NOLOCK)         
    INNER JOIN LibertyPower..Account A  WITH (NOLOCK)         
    ...
    WHERE C.ContractId    = @p_ContractID           
    and ac.AccountContractStatusID not in (2,3)    
    and A.CurrentContractId <> IsNull(A.CurrentRenewalContractId,0)  
    --End IT 121 Data Quality [Diogo Lima] 12/4/2013   
  ...
  BEGIN  
    ...
    SELECT @p_Status = ASS.[Status]
	FROM Libertypower..AccountStatus ASS WITH (NOLOCK)
	JOIN Libertypower..AccountContract AC WITH (NOLOCK)ON AC.AccountContractID =ASS.AccountContractID
   ...
   IF ((@p_SubmitEnrollmentFlag = 1) OR (NOT EXISTS( SELECT 1 FROM LibertyPower..WIPTask WITH (NOLOCK)
												     WHERE WIPTaskHeaderId = @p_WorkflowTaskHeaderID 
																	 AND TaskStatusId <> 3
																	 AND @p_WorkflowID <> @TaxExemptWorkflowId)
													 AND @p_Status ='01000'))       -- IT124 Agata Studzinska 12/20/2013
   BEGIN             
    UPDATE LibertyPower..AccountContract        
    SET SendEnrollmentDate = dateadd(dd, (-1) * U.EnrollmentLeadDays, C.StartDate)    ,    
    --IT 121 Data Quality [Rafael Vasques] 07/09/2013     
    AccountContractStatusID = 2 --Approved    
    FROM LibertyPower..AccountContract  AC WITH (NOLOCK)         
    INNER JOIN LibertyPower..Account A  WITH (NOLOCK)         
    ON AC.AccountId     = A.AccountId        
    AND AC.ContractId    = A.CurrentRenewalContractId        
    ...
    --BEGIN IT 121 Data Quality [Diogo Lima] 12/04/2013   
    UPDATE LibertyPower..AccountContract        
    SET  AccountContractStatusID = 2 --Approved    
    FROM LibertyPower..AccountContract  AC WITH (NOLOCK)         
    INNER JOIN LibertyPower..Account A  WITH (NOLOCK)         
    ON AC.AccountId     = A.AccountId        
    AND AC.ContractId    = A.CurrentContractId        
    ...
    WHERE C.ContractId    = @p_ContractID    
    and A.CurrentContractId <> IsNull(A.CurrentRenewalContractId,0)  
    --END IT 121 Data Quality [Diogo Lima] 12/04/2013   
  ...
  --Begin IT 121 Data Quality [Rafael Vasques] 07/09/2013      
    exec [usp_InsAccountSubmissionQueue] @p_ContractID    
  --End IT 121 Data Quality [Rafael Vasques] 07/09/2013      
