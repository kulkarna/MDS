...
-- Modification : Previous implementation of the SP was filtering accounts that didn't had an 
--				  entry in the AccountLatestService table because entries in this table are added 
--				  later by a service for the new contracts. Nonetheless we want to send 
--				  information for this given new accounts. Therefore the SP was modified to 
--                address the issue. Searching by account 777777777777999 (Pending) in ZipCode 
--                10001 (that was hidden), should now return an entry.
-- Format:		: DECLARE @zipCodes StringList;
--				  INSERT @zipCodes (Item) VALUES ('10001');
--				  EXEC usp_AccountsSummarySelect @zipCodes;
---------------------------------------------------------------------------------------------------

USE [Libertypower]
GO
/****** Object:  StoredProcedure [dbo].[usp_AccountsSummarySelect]    Script Date: 10/07/2014 17:04:33 ******/
SET ANSI_NULLS ON
GO
...
BEGIN 
	...
	FROM (
		...
		LEFT JOIN Libertypower..AccountLatestService ALS (NOLOCK) 
