...
BEGIN 
	...
	FROM (
		SELECT 
			...
			CASE WHEN ASt.Status= '905000' or ASt.Status= '906000' THEN 'Enrolled' ELSE 'Pending' End AS AccStatus, ASt.Status, 
			ACR.LegacyProductID, cp.IsDefault, S.ChannelName 
		FROM 
		...
		AND 
			S.ChannelID=C.SalesChannelID AND Ad.Zip IN (SELECT Item FROM @p_zipCodes) AND Ast.Status NOT IN ('911000','999998','999999')
		GROUP BY 
			A.AccountNumber, A.AccountIdLegacy, A.AccountID, A.AccountNameID, A.UtilityID, IsContractedRate, ALS.StartDate, 
			Als.EndDate, ASt.Status, ASt.SubStatus, ACR.LegacyProductID, cp.IsDefault, S.ChannelName
	) T1 FULL OUTER JOIN (
		SELECT 
		...
		AND 
			ALS.AccountID=A.AccountID AND ASt.AccountContractID=AC.AccountContractID AND cp.product_id=ACR.LegacyProductID 
		AND 
			Ad.Zip IN (SELECT Item FROM @p_zipCodes) AND ACR.IsContractedRate=1 AND Ast.Status NOT IN ('911000','999998','999999')
		GROUP BY 
			A.AccountNumber
