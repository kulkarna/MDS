 ...
 *******************************************************************************      
 * <12/2/2013> - <Rafael Vasques>        
 * Changing the first "IF" to work on the current kind of contract, since the dealtype of the contract dont  
 * change if the contract change from Renewal to Current.  
 *******************************************************************************     
 * <16/2/2013> - <Rafael Vasques>        
 * Changing the math of the ScheduledDate to use EnrollmentLeadTime instead of RateLeadtime.
 *******************************************************************************    
 */        
...
if exists (select 1 from account with(nolock) where CurrentRenewalContractId = @ContractId)         
 begin        
 ...
 begin        
  ...
  begin        
   ...
   select 2,3,1,dateadd(dd, -isnull(u.EnrollmentLeadDays,0), acr.RateStart),acr.RateStart,GETDATE(),AccountContractrateID         
   FROM LibertyPower..AccountContract  AC WITH (NOLOCK)             
   INNER JOIN LibertyPower..Account A  WITH (NOLOCK)             
   ON AC.AccountId     = A.AccountId            
   AND AC.ContractId    = A.CurrentRenewalContractId            
  ...
  begin        
   insert into  libertypower..AccountSubmissionQueue           
   select 2,4,1,dateadd(dd, -isnull(u.EnrollmentLeadDays,0), acr.RateStart),acr.RateStart,GETDATE(),AccountContractrateID         
   FROM LibertyPower..AccountContract  AC WITH (NOLOCK)             
   INNER JOIN LibertyPower..Account A  WITH (NOLOCK)             
   ON AC.AccountId     = A.AccountId            
   AND AC.ContractId    = A.CurrentRenewalContractId            
 ...
 begin        
  ...
  begin        
   insert into  libertypower..AccountSubmissionQueue           
   select 1,1,1,dateadd(dd, -isnull(u.EnrollmentLeadDays,0), acr.RateStart),acr.RateStart,GETDATE(),AccountContractrateID         
   FROM LibertyPower..AccountContract  AC WITH (NOLOCK)             
   INNER JOIN LibertyPower..Account A  WITH (NOLOCK)             
   ON AC.AccountId     = A.AccountId            
   AND AC.ContractId    = A.CurrentContractId            
  ...
  begin        
   insert into  libertypower..AccountSubmissionQueue           
   select 1,4,1,dateadd(dd, -isnull(u.EnrollmentLeadDays,0), acr.RateStart),acr.RateStart,GETDATE(),AccountContractrateID         
   FROM LibertyPower..AccountContract  AC WITH (NOLOCK)             
   INNER JOIN LibertyPower..Account A  WITH (NOLOCK)             
   ON AC.AccountId     = A.AccountId            
   AND AC.ContractId    = A.CurrentContractId            
