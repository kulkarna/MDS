 ...
 * 02/06/2014 - Pradeep Katiyar
 * Modified. Get the active instead of inactive.
 *******************************************************************************
...
Set @SQL='Select distinct pc.PromotionCodeId,pc.Code,pt.[Description] as PromotionType,pc.[Description],pc.MarketingDescription,pc.LegalDescription,
			u.Firstname + '' '' + u.Lastname as CreatedBy,pc.CreatedDate,case isnull(pc.Inactive,0) when 0 then ''Yes'' else ''No'' end InActive,
...
--SET NO_BROWSETABLE OFF
			...
			case isnull(c.Inactive,0) when 0 then 'Yes' else 'No' end Inactive,
			...
			where (c.Code like '%' + ltrim(rtrim(@p_CampaignCode)) + '%' or ltrim(rtrim(@p_CampaignCode))='') 
				and (ISNULL(c.InActive,0)= @p_InActive or @p_InActive is null)
