...
GO
IF EXISTS ( SELECT TOP 1 1 FROM sys.objects WITH (NOLOCK) WHERE name = 'usp_contract_pricing_val' AND type_desc = 'SQL_STORED_PROCEDURE')
DROP PROCEDURE [usp_contract_pricing_val];
GO
-- =============================================
...
-- Date : 7/17/2014 
-- Ticket: TFS 43699
-- Added check not to validate contract rate less than transfer rate for NJ market
-- =============================================
...
-- code restored for ticket 18855
   ...
   and ((select 1                        
         from lp_common..common_product 
         where product_id                           = @p_product_id 
         and   product_category                     ='FIXED') = 1) and @p_retail_mkt_id <> 'NJ'
   begin
      select @w_application                         = 'COMMON'
      select @w_error                               = 'E'
      select @w_msg_id                              = '00000045'
