...
/*---------------------------------------------------------------------------------------------------------------------------------------
 ...
 --------------------
 (1) Abhijeet Kulkarni (07/25/2013)
  Description: modified the join because the column 'IdrNonEdiHeaderId' has been removed from the table 'IdrAccountVsHuDiscrepancies'
  Added "nolock" where missing.
---------------------------------------------------------------------------------------------------------------------------------------*/
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN TRAN
			...
			inner join dbo.IdrAccountStageInfo s (nolock) on d.IdrAcctStageInfoId = s.Id
			where s.AccountNumber = @AccountNumber and s.UtilityCode = @UtilityCode
			
			-- IdrNonEdiDetail
			delete d
			from dbo.IdrNonEdiDetail d
			inner join dbo.IdrNonEdiHeader h (nolock) on h.ID = d.IdrAccountId  
			where h.AccountNumber = @AccountNumber and h.UtilityCode = @UtilityCode
			
			-- IdrAccountDetail
			delete d
			from dbo.IdrAccountDetail d
			inner join dbo.IdrAccountStageInfo h  (nolock) on h.ID = d.IdrAccountId  
			where h.AccountNumber = @AccountNumber and h.UtilityCode = @UtilityCode
