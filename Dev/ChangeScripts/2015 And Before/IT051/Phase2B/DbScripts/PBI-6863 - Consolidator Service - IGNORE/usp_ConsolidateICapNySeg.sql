...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'usp_ConsolidateICapNySeg')
    BEGIN
    	DROP PROCEDURE usp_ConsolidateICapNySeg
    END
GO


CREATE PROCEDURE usp_ConsolidateICapNySeg
AS
BEGIN
	SET NOCOUNT ON;
	...
	FROM LibertyPower.dbo.Account A WITH (NOLOCK)
		INNER JOIN LibertyPower.dbo.Utility U WITH (NOLOCK) ON A.UtilityID = U.ID AND U.UtilityCode = 'NySeg'
		...
		INNER JOIN Lp_Transactions.dbo.NySegAccount AA WITH (NOLOCK) ON A.AccountNumber = AA.AccountNumber 
