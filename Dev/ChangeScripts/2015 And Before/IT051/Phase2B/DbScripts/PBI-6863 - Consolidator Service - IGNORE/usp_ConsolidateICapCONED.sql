...
-- Modified Date: 3/13/2013
-- ************************************************************************************************

IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'usp_ConsolidateICapCONED')
    BEGIN
    	DROP PROCEDURE usp_ConsolidateICapCONED
    END
GO


CREATE PROCEDURE usp_ConsolidateICapCONED
AS
BEGIN
	SET NOCOUNT ON;
	...
	FROM LibertyPower.dbo.Account A WITH (NOLOCK)
		INNER JOIN LibertyPower.dbo.Utility U WITH (NOLOCK) ON A.UtilityID = U.ID AND U.UtilityCode = 'CONED'
		...
		LEFT JOIN Lp_Transactions.dbo.CONEDAccount AA WITH (NOLOCK) ON A.AccountNumber = AA.AccountNumber 
	WHERE (AI.AccountID IS NULL OR AA.Created > AI.StartDate ) 
		AND AA.ICAP >= 0 
