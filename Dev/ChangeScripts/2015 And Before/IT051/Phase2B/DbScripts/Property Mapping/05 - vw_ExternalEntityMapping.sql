...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_ExternalEntityMapping]'))
	DROP VIEW [dbo].[vw_ExternalEntityMapping]
GO
...
AS
	...
	FROM LibertyPower..ExternalEntityValue eev (NOLOCK)
		...
		LEFT JOIN LibertyPower..PropertyName p (NOLOCK)
