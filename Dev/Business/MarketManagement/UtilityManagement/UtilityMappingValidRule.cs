...
namespace LibertyPower.Business.MarketManagement.UtilityManagement
{
	...
	public class UtilityMappingValidRule : BusinessRule
	{
		...
		public UtilityMappingValidRule( int? identifier, int utilityID, int accountTypeID, int meterTypeID, int voltageID,
			string rateClassCode, string serviceClassCode, string loadProfileCode, string loadShapeCode, string tariffCode, string lossFactorID)
		...
		public UtilityMappingValidRule( int? identifier, int utilityID, int accountTypeID, int meterTypeID, int voltageID,
			string rateClassCode, string serviceClassCode, string loadProfileCode, string loadShapeCode, string tariffCode, string lossFactorID, string accountType, string voltageType, string meterType, string zone, decimal? icap = null, decimal? tcap = null )
			: base( "Utility Mapping Valid Rule", BrokenRuleSeverity.Error )
		{
			this.identifier = identifier;
			this.utilityID = utilityID;
		...
		public override bool Validate()
		{
			...
			if(_fieldMap.DeterminantField2 != TrackedField.Unknown && string.IsNullOrEmpty(_fieldMap.DeterminantValue2))
				AddException(new BrokenRuleException(this, "Determinant 2 must have a value", BrokenRuleSeverity.Error));
		...
		private bool IsAllElseMappingAndHasResultant( UtilityClassMappingDeterminantList determinantList )
		{
			bool hasResultant = (accountTypeID > -1 || meterTypeID > 0 || voltageID > 0 || !string.IsNullOrEmpty( rateClassCode ) || !string.IsNullOrEmpty( serviceClassCode )
								 || !string.IsNullOrEmpty(loadProfileCode) || !string.IsNullOrEmpty(loadShapeCode) || !string.IsNullOrEmpty(tariffCode) || !string.IsNullOrEmpty(lossFactorID)
			...
			foreach( var determinant in determinantList )
			{
				switch( determinant.Driver )
				{
					...
					case "ZoneID":
						...
						break;
					case "LossFactorID":
						if(!string.IsNullOrEmpty(lossFactorID))
							return false;
						break;
					case "LoadProfileID":
