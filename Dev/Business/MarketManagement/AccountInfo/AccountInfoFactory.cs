...
namespace LibertyPower.Business.MarketManagement.AccountInfo
{
	...
	public static class AccountInfoFactory
	{
        ...
        public static string AccountsInfoSelectZone(string accountNumber)
        {
            
            var dsAccount = AccountInfoSql.AccountsInfoSelect(accountNumber);

            if (!DataSetHelper.HasRow(dsAccount))
                return "";
            DataRow drAccount = dsAccount.Tables[0].Rows[0];
			return drAccount["OEZone"].ToString();
        }
		#endregion
