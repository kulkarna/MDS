...
namespace UsageEventAggregator.EventTransports.SqlServiceBroker
{
    public class ServiceBrokerTransport : ITransport
    {

        public const string ServiceBusTransportMessageContract = "ServiceBusTransportMessageContract";
        public const string ServiceBusTransportMessage = "ServiceBusTransportMessage";
        ...
        /// </remarks>
        public void ReceiveMessageLater(EventTransportMessage m)
        {
        ...
        public void Send(EventTransportMessage m, string destination)
        {
            GetSqlTransactionManager().RunInTransaction(transaction =>
            {
                // Always begin and end a conversation to simulate a monologe
                var conversationHandle = ServiceBrokerWrapper.BeginConversation(transaction, ReturnService, destination, ServiceBusTransportMessageContract);
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    ServiceBrokerWrapper.Send(transaction, conversationHandle, ServiceBusTransportMessage, stream.GetBuffer());
                }
                ServiceBrokerWrapper.EndConversation(transaction, conversationHandle);
            });
        }

        void SerializeTransportMessage(EventTransportMessage m, MemoryStream stream)
        {
        ...
        public int GetNumberOfPendingMessages()
        {
            ...
            GetSqlTransactionManager().RunInTransaction(transaction =>
            {
                count = ServiceBrokerWrapper.QueryMessageCount(transaction, InputQueue, ServiceBusTransportMessage);
            });
            return count;
        }
        ...
        private void Process()
        {
            ...
            try
            {
                Action processEx = () => GetSqlTransactionManager().RunInTransaction(ReceiveFromQueue);
        ...
        private void ReceiveFromQueue(SqlTransaction transaction)
        {
            ...
            try
            {
                ...
                if (message.MessageTypeName == ServiceBusTransportMessage)
                {
                    ...
                    EventTransportMessage eventTransportMessage = null;
                    try
                    {
                        eventTransportMessage = new BinaryFormatter().Deserialize(message.BodyStream) as EventTransportMessage;
                    }
                    catch (Exception e)
                    {
                    ...
                    var exceptionNotThrown = OnTransportMessageReceived(eventTransportMessage);
                    // and here
        ...
        private bool OnTransportMessageReceived(EventTransportMessage msg)
        {
            try
            {
        ...
        private void MoveToErrorService(Message message)
        {
            GetSqlTransactionManager().RunInTransaction(transaction =>
            {
                var conversationHandle = ServiceBrokerWrapper.BeginConversation(transaction, ReturnService, ErrorService, ServiceBusTransportMessageContract);
                ServiceBrokerWrapper.Send(transaction, conversationHandle, ServiceBusTransportMessage, message.Body);
                ServiceBrokerWrapper.EndConversation(transaction, conversationHandle);
            });
        }
