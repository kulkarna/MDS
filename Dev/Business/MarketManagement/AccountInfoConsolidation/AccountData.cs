...
namespace LibertyPower.Business.MarketManagement.AccountInfoConsolidation
{
	public class AccountData
	{
        ...
        /// 
        /// <returns> The string value of the object. Null if the object's value is -1 or empty</returns>
        protected string GetCleanCapValue(object cValue)
        {
            if (cValue != DBNull.Value && cValue.ToString().Trim() != string.Empty)
            {
 ...
 ////New code  for   PBI- 81211- Adding Reconciliation scripts for Account properties-
                //Invoke the Watchdoc Reconciliation script- This will be activated later as part of PBI-82454
                //bool watchdogSuccess = AccountFactory.AccountPropertyHistoryWatchdogInsert(AccountNumber, UtilityCode, 1);

                //if (!watchdogSuccess)
                //     Message += "AccountPropertyHistoryWatchdogInsert for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                // else
                //     Message += "AccountPropertyHistoryWatchdogInsert for account " + AccountNumber + @"/" + UtilityCode + " is complete.";
                
                //Invoke the profileRefId from EDI Reconciliation script
                bool accountUpdateLoadProfileRefIdModifiedFromEdiAccountLoadProfileSuccess = AccountFactory.AccountUpdateLoadProfileRefIdModifiedFromEdiAccountLoadProfile(AccountNumber);

                if (!accountUpdateLoadProfileRefIdModifiedFromEdiAccountLoadProfileSuccess)
                    Message += "AccountUpdateLoadProfileRefIdModifiedFromEdiAccountLoadProfile for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                else
                    Message += "AccountUpdateLoadProfileRefIdModifiedFromEdiAccountLoadProfile for account " + AccountNumber + @"/" + UtilityCode + " is complete.";
                
                //Invoke the LoadProfile  Reconciliation for the CONED script
                bool accountUpdateLoadProfileRefIdModifiedFromConedAccountLoadProfileSuccess = AccountFactory.AccountUpdateLoadProfileRefIdModifiedFromConedAccountLoadProfile(AccountNumber);

                if (!accountUpdateLoadProfileRefIdModifiedFromConedAccountLoadProfileSuccess)
                    Message += "AccountUpdateLoadProfileLoadProfileRefIdModifiedFromConedAccountLoadProfile for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                else
                    Message += "AccountUpdateLoadProfileLoadProfileRefIdModifiedFromConedAccountLoadProfile for account " + AccountNumber + @"/" + UtilityCode + " is complete.";
                
                //Invoke the LoadProfile Reconciliation for UI account script
                bool accountUpdateProfileForUIaccountsSuccess = AccountFactory.AccountUpdateProfileForUiaccounts(AccountNumber);

                if (!accountUpdateProfileForUIaccountsSuccess)
                    Message += "AccountUpdateProfileForUIaccounts for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                else
                    Message += "AccountUpdateProfileForUIaccounts for account " + AccountNumber + @"/" + UtilityCode + " is complete.";

                //Invoke the LoadProfile Reconciliation for Scraper Accounts
                bool accountUpdateScraperSuccess = AccountFactory.AccountUpdateScraper(AccountNumber);

                if (!accountUpdateScraperSuccess)
                    Message += "AccountUpdateScraper for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                else
                    Message += "AccountUpdateScraper for account " + AccountNumber + @"/" + UtilityCode + " is complete.";

                //Invoke the LocationRefId  Reconciliation script
                bool accountUpdateDeliveryLocationRefIdSuccess = AccountFactory.AccountUpdateDeliveryLocationRefId(AccountNumber);

                if (!accountUpdateDeliveryLocationRefIdSuccess)
                    Message += "AccountUpdateDeliveryLocationRefId for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                else
                    Message += "AccountUpdateDeliveryLocationRefId for account " + AccountNumber + @"/" + UtilityCode + " is complete.";

                //Invoke the ProfileREfId  Reconciliation script
                bool accountUpdateProfileRefIdSuccess = AccountFactory.AccountUpdateProfileRefId(AccountNumber);

                if (!accountUpdateProfileRefIdSuccess)
                    Message += "AccountUpdateProfileRefId for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                else
                    Message += "AccountUpdateProfileRefId for account " + AccountNumber + @"/" + UtilityCode + " is complete.";

                //Invoke the Zone Update  Reconciliation script for One zone Utilities
                bool accountUpdateOneZoneUtilitiesSuccess = AccountFactory.AccountUpdateOneZoneUtilities(AccountNumber);

                if (!accountUpdateOneZoneUtilitiesSuccess)
                    Message += "AccountUpdateOneZoneUtilities for account " + AccountNumber + @"/" + UtilityCode + " has failed";
                else
                    Message += "AccountUpdateOneZoneUtilities for account " + AccountNumber + @"/" + UtilityCode + " is complete.";
