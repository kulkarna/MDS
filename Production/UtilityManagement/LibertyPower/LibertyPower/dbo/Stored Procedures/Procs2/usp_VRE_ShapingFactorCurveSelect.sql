...
CREATE PROCEDURE [dbo].[usp_VRE_ShapingFactorCurveSelect]				
...
BEGIN
	...
	Begin
		Select 
			...
			[CreatedBy]
		From 
			VREShapingFactorCurve WITH (NOLOCK)
		Where	
	...
	Begin
		Select 
			...
			[CreatedBy]
		From 
			VREShapingFactorCurve C1 WITH (NOLOCK)
		Where	
			FileContextGUID =  ISNULL(@FileContextGuid,FileContextGuid) And			
			ID In 
			(
...
GO
EXECUTE sp_addextendedproperty @name = N'VirtualFolder_Path', @value = N'VRE', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_VRE_ShapingFactorCurveSelect';
