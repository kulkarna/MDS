...
-- Description:	Retreive all NYISO day ahead records
-- =============================================
CREATE PROCEDURE [dbo].[usp_VRE_NyisoDayAheadSelect]
...
BEGIN
	...
	BEGIN
		SELECT [ID]
		  ...
		  ,[DateModified]
		  ,[ModifiedBy]
		FROM [VRENyisoDayAhead] DA
		WHERE FileContextGUID = ISNULL(@FileContextGuid,FileContextGuid)
		AND	  ISNULL(@StartDate, DA.[TimeStamp]) <= DA.[TimeStamp]
		AND   ISNULL(@EndDate, DATEADD(DAY,1,DA.[TimeStamp])) > DA.[TimeStamp]
	...
	BEGIN
		...
		WHERE ID IN(
				SELECT MAX(DA.ID) 
				FROM [VRENyisoDayAhead] AS DA 
				WHERE FileContextGUID = ISNULL(@FileContextGuid,FileContextGuid)
				AND	 ISNULL(@StartDate, DA.[TimeStamp]) <= DA.[TimeStamp]
				AND   ISNULL(@EndDate, DATEADD(DAY,1,DA.[TimeStamp])) > DA.[TimeStamp]
				AND   ([Name] IN (SELECT zoneId FROM @tempZone) OR @useList = 0 )
				GROUP BY DA.Name, DA.[TimeStamp]
			);
	END
...
GO
EXECUTE sp_addextendedproperty @name = N'VirtualFolder_Path', @value = N'VRE', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_VRE_NyisoDayAheadSelect';
