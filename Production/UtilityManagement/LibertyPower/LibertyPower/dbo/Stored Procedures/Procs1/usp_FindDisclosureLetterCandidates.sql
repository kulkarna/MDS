...
-- for each contract that will expire in 30 days.
-- =============================================
CREATE PROCEDURE [dbo].[usp_FindDisclosureLetterCandidates]
...
BEGIN	
	...
	WHERE date_end <= DATEADD(day,30,@today)
	AND date_end >= @today
	AND p.isdefault = 0
...
GO
EXECUTE sp_addextendedproperty @name = N'VirtualFolder_Path', @value = N'LetterQueue', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_FindDisclosureLetterCandidates';
