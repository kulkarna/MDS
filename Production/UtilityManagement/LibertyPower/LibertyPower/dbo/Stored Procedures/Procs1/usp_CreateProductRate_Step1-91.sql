...
CREATE PROCEDURE [dbo].[usp_CreateProductRate_Step1-91]  (@EffDate datetime)
AS
BEGIN
	...
	DECLARE @DailyPricingEffDate DATETIME
	 SET @DailyPricingEffDate = DATEADD(dd,-1,@EffDate)
	
	-- The date we are looking for might be the current dailing pricing.
	-- Otherwise, look in the history table.
	IF EXISTS (SELECT * FROM ProductCrossPrice (NOLOCK) WHERE @DailyPricingEffDate BETWEEN CostRateEffectiveDate AND CostRateExpirationDate)
	BEGIN
		SELECT *
		INTO temp_pcp_all
		FROM ProductCrossPrice (NOLOCK)
		WHERE @DailyPricingEffDate BETWEEN CostRateEffectiveDate AND CostRateExpirationDate
	END
	ELSE
	BEGIN
		SELECT *
		...
		WHERE @DailyPricingEffDate BETWEEN CostRateEffectiveDate AND CostRateExpirationDate and channelgroupID = 91
	END
