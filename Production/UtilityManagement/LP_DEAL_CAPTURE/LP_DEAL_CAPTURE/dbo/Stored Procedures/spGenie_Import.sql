CREATE  proc [dbo].[spGenie_Import] as
...
*/
	...
	Begin
		...
		--IF EXISTS( SELECT 1 FROM [lp_deal_capture].[dbo].deal_contract_account (NOLOCK) WHERE  account_number =  @ServiceAccountNumber AND utility_id = @UtilityCode)
		--	SET @HasAccount=1;
...
--=====================
	...
	LEFT JOIN Libertypower..GeniePriceMapping m WITH (NOLOCK)
	ON a.ProductSelection = m.ProductID AND a.RateID = m.RateID
