...
CREATE procedure [dbo].[usp_contract_ins_jmunoz]
...
select @w_error                                     = 'I'
	...
	,@p_contract_nbr								= upper(@p_contract_nbr)
	...
	,@w_date_deal									= getdate()
	,@w_term_months									= 0
	,@w_username									= @p_username
	,@w_sales_channel_role							= ''
	,@w_contract_type								= ''
	,@w_grace_period								= 0
	,@w_contract_rate_type							= @p_contract_rate_type
	,@w_status										= 'DRAFT'
	,@w_action										= 'U'
	
select @w_contract_eff_start_date					= dbo.ufn_GetFirstDayNextMonth(GETDATE())	
set @w_date_created									= @w_date_deal
...
begin
	select @w_application                           = 'COMMON'
		...
		,@w_msg_id									= '00001014'
   		,@w_return									= 1
		,@w_descp_add								= ' (12 is Max)'
   goto goto_select
end

set @p_contract_nbr									= substring(@p_contract_nbr, 1, 12)
...
begin
   ...
   where Username                                   = @p_username
	...
	inner join libertypower..Roles b with (NOLOCK)
	...
	if ( SELECT	is_flexible FROM lp_common..common_product with (NOLOCK) WHERE product_id = @w_product_id ) = 1
		set @w_rate = 0
