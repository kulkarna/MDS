...
CREATE PROCEDURE [dbo].[usp_contract_pricing_ins_test]
...
print ' prate ' + convert(varchar(20),@p_rate)
print ' p transfer rate ' + convert(varchar(20),@p_transfer_rate) 

-- If @p_rate > @p_transfer_rate + @w_rate_cap AND @w_rate_cap > 0 AND @p_transfer_rate > 0

-- exec usp_contract_pricing_ins_test @p_username=N'libertypower\atafur',@p_contract_nbr=N'atat',@p_retail_mkt_id=N'PA',@p_utility_id=N'PPL',@p_account_type=N'1',@p_product_id=N'PPL_FX',@p_rate_id=N'444',@p_rate=N'0.081910',@p_contract_eff_start_date=N'07/01/2011',@p_enrollment_type=N'1',@p_requested_flow_start_date=N'07/01/2011',@p_term_months=N'18',@p_account_number=N'CONTRACT',@p_customer_code=N'',@p_customer_group=N'',@p_transfer_rate=N'0.06691',@p_HeatIndexSourceID=default,@p_HeatRate=default,@p_comm_cap=N'1',@p_contract_date=N'6/30/2011 12:00:00 AM'
...
WHERE	h.eff_date = @w_temp_contract_date
...
print 'checking the rate cap '

-- Rate Cap Validation SD# 10854 Gail Mangaroo 
-- ===============================
SET @w_rate_cap = 0 
SELECT @w_rate_cap = rate_cap FROM lp_deal_capture..deal_rate 

print ' w rate cap ' + convert(varchar(20),@w_rate_cap) 
...
begin
   ...
   update deal_contract set retail_mkt_id = @p_retail_mkt_id,
                            ...
                            date_end = case when @w_fixed_end_date = 1 
                                            then @w_due_date 
                                            else dateadd(mm, @w_term_months, @p_contract_eff_start_date) 
                                       end,
...
begin
   update deal_contract_account set contract_type = @w_contract_type,
                                    ...
                                    contract_eff_start_date = @p_contract_eff_start_date,
                                    term_months = @w_term_months,
                                    date_end = case when @w_fixed_end_date = 1 then @w_due_date else dateadd(mm, @w_term_months, @p_contract_eff_start_date) end,
                                    grace_period = case when grace_period = 0
                                                        then @w_grace_period
                                                        else grace_period
                                                   end,
