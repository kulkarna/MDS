...
CREATE function [dbo].[ufn_account_detail_JFORERO2]
...
-- ==================================================================================================       
       ...
       select A.AccountIDLegacy AS account_id,
			...
			CASE WHEN A.CurrentRenewalContractID IS NOT NULL 
				 THEN convert(varchar(10), LibertyPower.dbo.ufn_GetLegacyFlowStartDate(AST.[Status],AST.[SubStatus],ASERVICE.StartDate), 101)
			ELSE	case when LibertyPower.dbo.ufn_GetLegacyFlowStartDate(AST.[Status],AST.[SubStatus],ASERVICE.StartDate) < '1/1/1901' then 'Not Yet Available'
					else lp_enrollment.dbo.ufn_date_format(LibertyPower.dbo.ufn_GetLegacyFlowStartDate(AST.[Status],AST.[SubStatus],ASERVICE.StartDate),'<Month> <DD> <YYYY>') end 
			END
			AS date_flow_start,
		...
		WHERE 
		A.CurrentContractID IS NOT NULL 
		OR 
		A.CurrentRenewalContractID IS NOT NULL
