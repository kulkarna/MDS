...
CREATE FUNCTION [dbo].[ufn_custom_rates_sel] ( @Username	nchar(100) )
RETURNS Table
AS
...
(
	...
	FROM	lp_common..common_product_rate pr WITH (NOLOCK)
			JOIN lp_deal_capture..deal_pricing_detail dpd WITH (NOLOCK) ON dpd.product_id		= pr.product_id  AND dpd.rate_id			= pr.rate_id 
			JOIN lp_deal_capture..deal_pricing dp WITH (NOLOCK) ON dp.deal_pricing_id	= dpd.deal_pricing_id
	WHERE 1=1
	...
	AND		(	-- user has access to custom rate	
				...
				(	SELECT	b.RoleName 
					FROM	libertypower..[UserRole] a WITH (NOLOCK)
							...
							JOIN libertypower..[User] u WITH (NOLOCK) ON a.userID = u.UserID
