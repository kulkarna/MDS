...
{
    public class MeterReadCalendarBusinessLayer : SheetBusinessLayer
    {
        #region private variables
        private const string CLASS = "MeterReadCalendarBusinessLayer";
        private const string TABLE_ONE_NAME = "Meter Read Calendar";
        #endregion

        #region public constructors
        public MeterReadCalendarBusinessLayer(IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger)
        ...
        protected override DataSet GetTableOneDataSetAll(string messageId)
        {
            ...
            try
            {
                ...
                DataSet dataSet = _repository.usp_MeterReadCalendar_GetAll(messageId);
        ...
        protected override DataSet GetTableOneDataSet(string messageId, string utilityCode)
        {
            ...
            try
            {
                ...
                DataSet dataSet = _repository.usp_MeterReadCalendar_GetByUtilityCode(messageId, utilityCode);
                _logger.LogInfo(messageId, string.Format("{0}.{1}.{2} dataSet.Tables[0].TableName = TABLE_ONE_NAME;", NAMESPACE, CLASS, method));
                dataSet.Tables[0].TableName = TABLE_ONE_NAME;
        ...
        internal override void InsertDataTables(string messageId, DataSet dataSet, string userName)
        {
            ...
            try
            {
                ...
                _tabsBusinessLayer = new MeterReadCalendarTabsBusinessLayer(messageId, _repository, _excelWorksheetUtility, _logger, dataSet, userName);
                _logger.LogInfo(messageId, string.Format("{0}.{1}.{2} _tabsBusinessLayer = new MeterReadCalendarTabsBusinessLayer(messageId, _repository, _excelWorksheetUtility, _logger, dataSet)", NAMESPACE, CLASS, method));
