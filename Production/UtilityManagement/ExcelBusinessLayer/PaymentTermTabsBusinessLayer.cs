...
{
    public class PaymentTermTabsBusinessLayer : TabsBusinessLayer
    {
        #region private variables
        private string CLASS = "PaymentTermTabsBusinessLayer";
        #endregion

        #region public constructors
        public PaymentTermTabsBusinessLayer(string messageId, IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger, DataSet dataSet, string userName)
            : base(messageId, dataRepository, excelWorksheetUtility, logger, dataSet, userName)
        {
            string method = string.Format("PaymentTermTabsBusinessLayer(messageId,dataRepository,excelWorksheetUtility,logger,dataSet,userName:{0})",userName);
            try
            {
                ...
                if (_dataSet != null && _dataSet.Tables != null && _dataSet.Tables.Count > 0 && _dataSet.Tables[0] != null && _dataSet.Tables[0].Rows != null)
                {
                    foreach (DataTable dataTable in _dataSet.Tables)
                    {
                        if (dataTable != null && !string.IsNullOrWhiteSpace(dataTable.TableName))
                        {
                            ...
                            {
                                case "Payment Term":
                                    ITabBusinessLayer paymentTermTabBusinessLayer = new PaymentTermTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(paymentTermTabBusinessLayer);
                                    break;
                            }
                        }
                    }
