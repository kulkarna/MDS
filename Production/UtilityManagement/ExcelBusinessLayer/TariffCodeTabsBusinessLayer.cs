...
{
    public class TariffCodeTabsBusinessLayer : TabsBusinessLayer
    {
        #region private variables
        private string CLASS = "TariffCodeTabsBusinessLayer";
        #endregion

        #region public constructors
        public TariffCodeTabsBusinessLayer(string messageId, IDataRepository dataRepository, IExcelWorksheetUtility excelWorksheetUtility, ILogger logger, DataSet dataSet, string userName)
            : base(messageId, dataRepository, excelWorksheetUtility, logger, dataSet, userName)
        {
            string method = string.Format("TariffCodeTabsBusinessLayer(messageId,dataRepository,excelWorksheetUtility,logger,dataSet,userName:{0})",userName);
            try
            {
                ...
                if (_dataSet != null && _dataSet.Tables != null && _dataSet.Tables.Count > 0 && _dataSet.Tables[0] != null && _dataSet.Tables[0].Rows != null)
                {
                    foreach (DataTable dataTable in _dataSet.Tables)
                    {
                        if (dataTable != null && !string.IsNullOrWhiteSpace(dataTable.TableName))
                        {
                            ...
                            {
                                case "Tariff Code":
                                    ITabBusinessLayer tariffCodeTabBusinessLayer = new TariffCodeTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(tariffCodeTabBusinessLayer);
                                    break;
                                case "LP Std Tariff Code":
                                    ITabBusinessLayer lpStandardTariffCodeTabBusinessLayer = new LpStandardTariffTabCodeBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(lpStandardTariffCodeTabBusinessLayer);
                                    break;
                                case "Tariff Code Alias":
                                    ITabBusinessLayer tariffCodeAliasTabBusinessLayer = new TariffCodeAliasTabBusinessLayer(messageId, _dataRepository, _excelWorksheetUtility, _logger, dataTable, UtilityCompanies);
                                    TabBusinessLayerList.Add(tariffCodeAliasTabBusinessLayer);
                                    break;
                            }
                        }
