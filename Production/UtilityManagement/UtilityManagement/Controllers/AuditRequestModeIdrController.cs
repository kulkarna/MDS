...
namespace UtilityManagement.Controllers
{
    ...
    public class AuditRequestModeIdrController : ControllerBaseWithoutUtilDropDown
    {
        ...
        public AuditRequestModeIdrController() : base()
        {
            ViewBag.PageName = "AuditRequestModeIDR";
            ViewBag.IndexPageName = "AuditRequestModeIDR";
            ViewBag.PageDisplayName = "Audit Request Mode IDR";
        }
        #endregion
        ...
        public ActionResult Index()
        {
            ...
            catch (Exception exc)
            {
                ...
                return View(new List<usp_zAuditRequestModeIdr_SELECT_Result>());
            }
            //return View(db.zAuditRequestModeIdrs.ToList());
        }
        ...
        {
            var response = _db.usp_zAuditRequestModeIdr_SELECT().OrderBy(x => x.Id).ToList();
            ...
            switch (Session[Common.SORTCOLUMNNAME].ToString())
            {
                ...
                case "UtilitysSlaHistoricalUsageResponseInDays":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.UtilitysSlaIdrResponseInDays).ToList();
                    else
                        response = response.OrderBy(x => x.UtilitysSlaIdrResponseInDays).ToList();
                    break;
                case "UtilitysSlaHistoricalUsageResponseInDaysPrevious":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.UtilitysSlaIdrResponseInDaysPrevious).ToList();
                    else
                        response = response.OrderBy(x => x.UtilitysSlaIdrResponseInDaysPrevious).ToList();
                    break;
                case "LibertyPowersSlaFollowUpHistoricalUsageResponseInDays":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.LibertyPowersSlaFollowUpIdrResponseInDays).ToList();
                    else
                        response = response.OrderBy(x => x.LibertyPowersSlaFollowUpIdrResponseInDays).ToList();
                    break;
                case "LibertyPowersSlaFollowUpHistoricalUsageResponseInDaysPrevious":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
                        response = response.OrderByDescending(x => x.LibertyPowersSlaFollowUpIdrResponseInDaysPrevious).ToList();
                    else
                        response = response.OrderBy(x => x.LibertyPowersSlaFollowUpIdrResponseInDaysPrevious).ToList();
                    break;
                case "IsLoaRequired":
                    if (Session[Common.SORTDIRECTION].ToString() == Common.DESC)
        ...
        private List<usp_zAuditRequestModeIdr_SELECT_Result> ObtainResponse()
        {
            var response = _db.usp_zAuditRequestModeIdr_SELECT().OrderBy(x => x.Id).ToList();
            if (Session[Common.SORTCOLUMNNAME] == null)
                Session[Common.SORTCOLUMNNAME] = "Id";
