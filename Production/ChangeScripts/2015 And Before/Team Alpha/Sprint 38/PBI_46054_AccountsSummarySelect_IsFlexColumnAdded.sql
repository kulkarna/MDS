...
BEGIN 
	...
	SELECT 
		...
		T1.UtilityID, T1.ChannelName AS SalesChannel, T1.IsDefault, T1.is_flexible AS IsFlex
	FROM (
		SELECT 
			...
			AC.AccountContractStatusID, ACR.LegacyProductID, cp.IsDefault, S.ChannelName, cp.is_flexible 
		FROM 
		...
		GROUP BY 
			...
			Als.EndDate, AC.AccountContractStatusID, ASt.SubStatus, ACR.LegacyProductID, cp.IsDefault, S.ChannelName, cp.is_flexible 
	) T1 FULL OUTER JOIN (
		SELECT 
