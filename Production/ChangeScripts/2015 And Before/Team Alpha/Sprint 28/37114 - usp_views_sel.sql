...
Modified ON: 4/14/2014
Modified by: Pradeep Katiyar
TFS 37114 - As a Channel Manager I need to make sure the tablet users that are created using the Tablet Sales Agent Administration tool have the proper minimum permissions to be allowed to enter contracts so that the Sales Agents can submit contracts using the Tablet
Reason - Commented the check where it was returning the sales channels having the user id  same as sales channel name.
*/
------------------------------------------------------------------------------------------------------
	...
	ELSE
		INSERT INTO #List_of_sales_channels
			...
			WHERE 
				...
				--TFS 37114- Commented the check so that sales agent users having userid other than sales channel name can submit the contract.
