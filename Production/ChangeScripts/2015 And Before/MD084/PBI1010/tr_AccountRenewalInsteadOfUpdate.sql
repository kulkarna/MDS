...
-- =============================================
ALTER TRIGGER [dbo].[tr_AccountRenewalInsteadOfUpdate]
...
BEGIN
	...
	DECLARE inserted_renewal_cursor CURSOR FOR 
	SELECT [account_id]
      ,[account_number]
      ,[account_type]
      ,[status]
	...
	BEGIN
		...
		*/
		...
		BEGIN
			...
			------ Multi-term MD084 ---------------------------------------------------------------------
			DECLARE @ProductCrossPriceMultiID int

			SELECT	TOP(1) @ACR_AccountContractRateID = AccountContractRateID, @ProductCrossPriceMultiID = ProductCrossPriceMultiID
			FROM	LibertyPower.dbo.AccountContractRate ACR (NOLOCK)
			WHERE	ACR.AccountContractID = @AC_AccountContractID
			ORDER BY AccountContractRateID;
