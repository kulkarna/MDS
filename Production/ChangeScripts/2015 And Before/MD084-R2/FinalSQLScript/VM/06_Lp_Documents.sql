USE [lp_documents]
GO
/****** Object:  StoredProcedure [dbo].[usp_document_print_template_table_mulitrate_sel]    Script Date: 01/16/2013 12:54:40 ******/
SET ANSI_NULLS ON
GO
...
-- ==================================================================================
-- 11/2/2012 Lev Rosenblum added functionality for multi-term products into contract-print section
-- ==================================================================================
-- 11/27/2012 Lev Rosenblum 
-- move cursor at the top of procedure: new output applied for all cases
-- add DefaultSubRateList to output (should be used later for PA market
-- ==================================================================================
-- 11/29/2012 Lev Rosenblum 
-- create @PriceId for each 'IF' statement due to unique table for selecting a priceid
-- ==================================================================================
-- 12/17/2012 Lev Rosenblum 
-- new functionality has been added related to Bug 4497 (No priceId selected)
-- ==================================================================================
-- 1/16/2013 Lev Rosenblum 
-- case statement has been added for term_months in few use-cases 
--(Bug SR1-5338800?: No Total Term information were displayed before)
-- ==================================================================================
...
BEGIN
	...
	-----------------Bug 4498 implementation start here------------	
	DECLARE @ShowRate					bit
	DECLARE @NotFromPrintContractScreen	bit
	SET @NotFromPrintContractScreen=1
	-----------------Bug 4498 implementation end here------------	
	------------------------------------------------------------------------------------------
	...
	BEGIN 
		...
		SET @NotFromPrintContractScreen=0
...
FINAL: 
	...
	-----------------Bug 4498 implementation start here------------	
	SELECT mr.account_number, convert(decimal(13,5),mr.rate) as rate, cmnP.is_flexible as showRate
	FROM @temp_multi_rate mr 
		INNER JOIN lp_common..common_product cmnP with (nolock)
			ON cmnP.product_id=mr.product_id
	---------------Bug 4498 implementation end here------------	
	...
	---------------Bug 4498 implementation end here------------	
		...
		BEGIN
			...
			IF EXISTS(	
				...
				BEGIN
					...
					WHERE rtrim(contract_nbr)=@contract_number and rtrim(account_number)=@AccountNumber
					print 'renew contract_account 1'
					GOTO CONTINUING;			
				END
			...
			IF EXISTS(	
				...
				BEGIN
					SELECT @priceId = PriceID
					FROM lp_contract_renewal.dbo.deal_contract_account with (nolock)
					WHERE contract_nbr=@contract_number and account_number=@AccountNumber
					print 'renew contract_account 2'	
					GOTO CONTINUING;			
				END
			...
			IF EXISTS(	
				...
				BEGIN
					...
					WHERE contract_nbr=@contract_number
					print 'renew contract_account 3'
					GOTO CONTINUING;
				END
			...
			IF EXISTS(	
				...
				BEGIN
					SELECT @priceId = PriceID 
					FROM lp_deal_capture.dbo.deal_contract_account with (nolock)
					WHERE contract_nbr=@contract_number and account_number=@AccountNumber
					print 'new contract_account 2'
					GOTO CONTINUING;
				END
					
			SELECT @priceId= PriceID
			FROM lp_deal_capture..deal_contract_print WITH (NOLOCK)
			WHERE contract_nbr=@contract_number and RateString is not null
		...
		CONTINUING:
			...
			SET @Count=1;
			SET @TermLength=0;
			IF (@NotFromPrintContractScreen=1) 
			BEGIN
				SET @ShowRate=0
			END
			IF (@priceId>0)
			BEGIN
				...
				BEGIN
					...
					BEGIN
						IF @RateStatement=''
							BEGIN
								SET @RateStatement='Sub-term #' + Convert(varchar(2),@Count)+': $_______/kWh*';
							END
						ELSE 
							SET @RateStatement=@RateStatement + char(13)+ 'Sub-term #' + Convert(varchar(2),@Count)+': $_______/kWh*';
					END	
					ELSE
					BEGIN
						IF @RateStatement=''
							BEGIN
								IF (@AssignedRate>0) SET @Delta=@AssignedRate-@Rate
								ELSE SET @Delta=0
								SET @RateStatement='Sub-term #' + Convert(varchar(2),@Count)+': $'+ Convert(varchar(7), @Rate+@Delta) +'/kWh*';
							END
						ELSE 
							SET @RateStatement=@RateStatement + char(13)+ 'Sub-term #' + Convert(varchar(2),@Count)+': $'+ Convert(varchar(7), @Rate+@Delta)+'/kWh*';
					END
			...
			END
			------------ Bug 4497 implementation start here ---------------------
			ELSE
			BEGIN
				...
				FROM lp_deal_capture..deal_contract_print WITH (NOLOCK)
				WHERE contract_nbr=@contract_number and RateString is null
				...
				BEGIN
					...
					ELSE 
						SET @TermStatement=@TermStatement + char(13)+ 'Sub-term #' + Convert(varchar(2),@Count)+': Months '+ Convert(varchar(2), @TermLength+1)+' to '+ Convert(varchar(2), @TermLength+CONVERT(int,@TempString));
				...
				END
			END
			------------ Bug 4497 implementation end here ---------------------
	...
	SELECT DISTINCT
		...
		, ISNULL((
					...
					from lp_account..account_info with (nolock)
		...
		, ISNULL((
					...
					from lp_account..account_meters with (nolock)
					where account_id = a.account_id ),
				'')															AS MeterNumber
