namespace LibertyPower.Business.MarketManagement.EdiParser.FileParser
{
	...
	/// </summary>
	public class MecoMapper867 : MapperBase
	{
		/// <summary>
		/// Default constructor
		/// </summary>
		public MecoMapper867() { }
		...
		public MecoMapper867( string utilityCode, string marketCode )
		{
		...
		{
			MecoMarker mecoMarker = new MecoMarker();
				...
				else
				{
					switch( marker )
					{
						case "N18S": // duns number
							{
								cellContents = cells[mecoMarker.DunsNumberCell].Trim();
						...
						case "BPT52":
							{
								cellContents = cells[mecoMarker.TransactionSetPurposeCodeCell].Trim();
								transactionSetPurposeCode = cellContents == null ? "" : cellContents;
                                cellContents = "";
                                if (cells.Length > 3)
                                {
                                    string dateString = string.Empty;
                                    dateString = cells[mecoMarker.TransactionCreationDateCell].Trim();
						...
						case "PSA93": // icap
							{
								cellContents = cells[mecoMarker.IcapCell].Trim();
								icap = cellContents == null ? "" : cellContents;
								break;
							}
						case "QTYKZ": // tcap
							{
								cellContents = cells[mecoMarker.TcapCell].Trim();
								tcap = cellContents == null ? "" : cellContents;
								break;
							}
						case "REFSPL": // zone
							{
								cellContents = cells[mecoMarker.ZoneCell].Trim();
								zone = cellContents == null ? "" : cellContents;
								break;
							}
						case "REFBF":	//bill cycle
							{
								cellContents = cells[mecoMarker.BillGroupCell].Trim();
								billGroup = cellContents == null ? "" : cellContents;
								break;
							}
						case "REFLO":	//load profile
							{
								cellContents = cells[mecoMarker.LoadProfileCell].Trim();
						...
						case "MEA": // kwh, uom, measurement significance code
							{
								cellContents = cells[mecoMarker.QuantityCell].Trim();
								quantity = cellContents == null ? "" : cellContents;

								cellContents = cells[mecoMarker.UnitOfMeasurementCell].Trim();
								unitOfMeasurement = cellContents == null ? "" : cellContents;

								cellContents = cells[mecoMarker.MeasurementSignificanceCodeCell].Trim();
						...
						case "DTM150": // begin date
							{
								cellContents = cells[mecoMarker.BeginDateCell].Trim();
						...
						case "DTM151": // end date
							{
								cellContents = cells[mecoMarker.EndDateCell].Trim();
						...
						case "QTYQD": // begin usage
							{
								...
								if( !fc.Contains( "NV" ) )
								{
									cellContents = cells[mecoMarker.QuantityAltCell].Trim();
									quantity = cellContents == null ? "" : cellContents;

									cellContents = cells[mecoMarker.UnitOfMeasurementAltCell].Trim();
									unitOfMeasurement = cellContents == null ? "" : cellContents;
