namespace LibertyPower.Business.MarketManagement.EdiParser.FileParser
{
	...
	/// </summary>
	public class AepceMapper867 : MapperBase
	{
		/// <summary>
		/// Default constructor
		/// </summary>
		public AepceMapper867() { }
		...
		public AepceMapper867( string utilityCode, string marketCode )
		{
		...
		{
			AepceMarker aepceMarker = new AepceMarker();
			...
			foreach( string fc in fileCellList )
			{
				...
				switch( cell0 )
				{
					...
					case "PTD":													// new usage record..
						{
							...
							cellContents = cells[aepceMarker.PtdLoopCell].Trim();
					...
					case "BPT": // transaction set purpose code
						{
							cellContents = cells[aepceMarker.TransactionSetPurposeCodeCell].Trim();
							transactionSetPurposeCode = cellContents == null ? "" : cellContents;
                            cellContents = "";
                            if (cells.Length > 3)
                            {
                                string dateString = string.Empty;
                                dateString = cells[aepceMarker.TransactionCreationDateCell].Trim();
				...
				switch( marker )
				{
					case "N18S": // duns number
						{
							cellContents = cells[aepceMarker.DunsNumberCell].Trim();
					...
					case "N18R": // customer name
						{
							cellContents = cells[aepceMarker.CustomerNameCell].Trim();
							customerName = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFQ5": // account number
						{
							cellContents = cells[aepceMarker.AccountNumberCell].Trim();
							accountNumber = cellContents == null ? "" : cellContents;
							break;
						}
					case "REF45": // previous account number
						{
							cellContents = cells[aepceMarker.PreviousAccountNumberCell].Trim();
							previousAccountNumber = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFMG": // meter number
						{
							cellContents = cells[aepceMarker.MeterNumberCell].Trim();
							meterNumber = cellContents == null ? "" : cellContents;
							break;
						}
                    case "PTDPM": // meter number
                        {
                            cellContents = cells[aepceMarker.IdrMeterNumberCell].Trim();
                            meterNumber = cellContents == null ? "" : cellContents;
                            ptdLoop = cells[aepceMarker.PtdLoopCell].Trim();
                            break;
                        }
					case "REFNH": // rate class
						{
							cellContents = cells[aepceMarker.RateClassCell].Trim();
							rateClass = cellContents == null ? "" : cellContents;
							break;
						}
					case "QTYKC": // icap
						{
							cellContents = cells[aepceMarker.IcapCell].Trim();
							icap = cellContents == null ? "" : cellContents;
							break;
						}
					case "QTYKZ": // tcap
						{
							cellContents = cells[aepceMarker.TcapCell].Trim();
							tcap = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFSPL": // zone
						{
							cellContents = cells[aepceMarker.ZoneCell].Trim();
							zone = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFBF":	//bill cycle
						{
							cellContents = cells[aepceMarker.BillGroupCell].Trim();
							billGroup = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFLO":	//load profile
						{
							cellContents = cells[aepceMarker.LoadProfileCell].Trim();
							loadProfile = cellContents == null ? "" : cellContents;
							break;
						}
					case "REFMT": // usage type
						{
							cellContents = cells[aepceMarker.UsageTypeCell].Trim();
                   ...
                   // case "MEAAF":
						{
							...
							{
								cellContents = cells[aepceMarker.QuantityCell].Trim();
								quantity = cellContents == null ? "" : cellContents;

								cellContents = cells[aepceMarker.UnitOfMeasurementCell].Trim();
								unitOfMeasurement = cellContents == null || cellContents.Length.Equals( 0 ) ? usageType : cellContents;

								if( aepceMarker.MeasurementSignificanceCodeCell < cells.Length )
								{
									cellContents = cells[aepceMarker.MeasurementSignificanceCodeCell].Trim();
					...
					case "DTM194": // idr date
						{
							cellContents = cells[aepceMarker.IdrDateCell].Trim();
							idrDate = cellContents == null ? "" : cellContents;

							cellContents = cells[aepceMarker.IdrIntervalCell].Trim();
					...
					case "DTM514": // exchange meter date
						{
							cellContents = cells[aepceMarker.BeginDateCell].Trim();
					...
					case "DTM151": // end date
						{
							cellContents = cells[aepceMarker.EndDateCell].Trim();
					...
					case "QTYQD": // begin usage
						{
							cellContents = cells[aepceMarker.QuantityAltCell].Trim();
							quantity = cellContents == null ? "" : cellContents;
							break;
						}
                    case "QTY20":
